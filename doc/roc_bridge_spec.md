...
**Why this mode earns its place, given the viewer can mostly do it alone.** The viewer's own path costs real sim-visible traffic: one `ParcelPropertiesRequest` per parcel per visit to learn each parcel's owner, group and auto-return timer, plus a batched `ObjectSelect` probe for every object whose "set to group" status is unknown — because there is no group-set bit anywhere in the object update, and `FLAGS_OBJECT_GROUP_OWNED` means *deeded*, which is a different thing. Region-wide parcel enumeration plus bulk object selects, running automatically on every region entry, is a scraper-shaped traffic profile that an estate's anti-copybot heuristics may well flag.

**One `a` sweep replaces that entire apparatus** where the bridge is available: no parcel enumeration, no select probes, no selection-adjacent messages at all — just one HTTP round trip to a script the user is already wearing. That is a large reduction in both sim load and griefing signature, which is why the mode is worth the script cycles despite the viewer having a fallback. The viewer keeps its own path for OpenSim, bridge-off and script-mute estates.
