...
// <SS:Nexii> NOTHING CALLS THIS YET. The consolidation - rewiring the three shipped call sites onto these formulas - lands as a separate step once the core is proven, so do not assume the shaders already agree with it because this file exists. What IS proven today: V:\Scratch\atmo\tests\twin_airlight.cpp transliterates skyV.glsl's and cloudsV.glsl's arithmetic from their TEXT and asserts this core reproduces both, and V:\Scratch\atmo\tests\unit_airlight.cpp pins the invariants 8c depends on. Until the rewire, a change here changes nothing on screen.
