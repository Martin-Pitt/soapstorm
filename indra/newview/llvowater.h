...
public:
    ...
    // <SS:Nexii> Atmo Magic's duplicate water planes (sswater.h) reuse the stock water pcodes so every pcode-keyed check treats the families identically; this flag is the only thing telling
    // them apart, read by the draw-time family gate SSWaterWorld::drawsThisFrame (doc/atmo_magic_water.md).
