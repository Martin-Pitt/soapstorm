...
// Fixed in code: every client has to walk the same streams, so this is not a user setting.
...
// Track crossfade rates, per second. Precipitation swaps need to be quick enough not to feel like a bug and slow enough to read as weather; wind eases faster because the audio bed follows it
// directly.
static const F32 TRACK_FADE_RATE = 0.45f;
static const F32 WIND_FADE_RATE  = 0.8f;

// Wrap on the accumulated wind drift, metres. Far outside anything a gust pattern is sampled across, and a multiple of the region grid.
...
void SSAtmoMagic::refreshParams()
{
    ...
    // the v2 per-track weather layer entirely for this frame - see doc/atmo_magic_environment.md's untangle plan. v3_cfg is left untouched and v3_active false whenever nothing is loaded, so the v2
    // path below runs completely unmodified for anyone who hasn't opted into v3. world_z/prev_world_z/teleported are SSAtmoEnvTrackResolver's own inputs (see ssatmoenvtrackstate.h) - tracked here
    // frame to frame since nothing else in the viewer already carries "where was the avatar a moment ago". A region change or an implausibly large single-frame jump both count as "teleported", which
    // is meant to also catch sit-teleport and region-position-change per the design doc's instant-cut rule - there is no dedicated completion event for any of those this hooks into instead.
    ...
    // Weather is configured per EEP sky track and only runs for the track the camera is in. Nothing is defined by default, so a track without a config stays clear no matter what the master switch
    // says.
    ...
    // The preset this track asks for; an empty or unrecognised name falls back to whatever the preset editor currently has selected.
    ...
    // Crossfade. A preset cannot be interpolated, so precipitation eases to nothing before a swap and back up afterwards: crossing a band boundary, or a notecard arriving mid-session, reads as the
    // weather changing rather than teleporting.
    ...
    // Gust shape, folded into one depth figure with the turbulence it scales by. Eased along with the crossfade so crossing a band boundary does not step from a steady wind into a squall between
    // frames.
    ...
    // Ease the wind vector rather than the orientation, so a direction change cannot sweep the long way round the compass
    ...
    // How far the air itself has travelled. Anything keyed to a pattern that rides along with the wind - the flowmap's gust waves - phases off this rather than off speed times the clock, which would
    // jump the whole field bodily every time the wind eased to a different speed. Seeded from the shared clock so two clients that have been standing in the same steady wind agree on where in the
    // cycle it is; after that they only diverge by the history of speed changes they each saw. Wrapped a long way out to keep the single precision the field is sampled at, at the cost of one phase
    // jump per several hours of hard wind.
    if (mWindDriftSeeded)
    {
    ...
    // Ground zero for this track: terrain and water at ground level, the band's own base altitude up in the sky unless the config pins it to a platform. (cfg.mHasGround is unconditionally true for a
    // v3-resolved config, so tracks->trackFloor(mTrack) - which would read v2's meaningless-to-v3 track number - is never actually reached in that case.)
    ...
    // Mean fall direction: wind tilt over fall speed; slow types drift hard. Rising types keep a near-vertical axis so the shadow map stays usable as a ground height map for them.
...
F32 SSAtmoMagic::gustEnvelopeAt(F64 time) const
{
    ...
    // Slow waves plus a sharpened burst channel; both pure functions of the shared clock so gust timing lines up across clients. fbm output hovers well inside [-1,1], so both channels are stretched
    // hard — otherwise the envelope barely moves and gusts are imperceptible.
    ...
    // At full turbulence rain arrives in real waves: near-lulls between gusts, more than double strength inside them
...
{
    // Keyed on the preset name plus its asset strings, so switching or editing a preset re-parses and anything else is a cheap no-op
...
LLViewerTexture* SSAtmoMagic::textureFor(const SSAtmoAsset& asset, LLColor4& tint, F32& glow)
{
    if (asset.mIsPBR)
    {
        // Approximated through the legacy particle renderer: base color texture, factor as tint, emissive strength as glow. A dedicated PBR particle draw pool can take over here later.
...
void SSAtmoMagic::shift(const LLVector3& offset)
{
    ...
    // Queued edits are agent-space too. Left unshifted they would mature into a dirty mark a region's width away from the object that caused it, and every one of them would look like it had moved on
    // the next settle pass and re-arm, so a region crossing would both miss the real geometry and stall the queue.
...
void SSAtmoMagic::queueImpact(F64 time, const LLVector3& pos_agent, F32 strength, bool on_water,
                              ...
                              bool from_runoff)
{
    // Bounded so a hitch or bad parameters can't grow this without limit. The bound has to hold everything in flight, not everything landing this second: an impact is queued when its drop is spawned
    // and dispatched when it arrives, so the queue depth is the landing rate times the whole fall time. Rain falling 16 to 26 metres at 9.5 m/s is better than two seconds in the air, and a few
    // thousand landings a second across the impact radius puts ten thousand entries in here in steady state. The old bound of two thousand was reached almost immediately and silently threw the rest
    // away, which is why the ripples read as a fraction of the drops.
...
void SSAtmoMagic::processImpacts()
{
    ...
    while (!mImpacts.empty() && mImpacts.begin()->first <= mNow)
    {
        ...
        // Feeds the ambient loop loudness: sheltered spots see fewer landings and sound quieter at the same weather parameters
        SSSoundscape::getInstance()->notifyImpact(
            impact.mStrength * (1.f - dist / IMPACT_SEE_RADIUS));

        // Deterministic per-impact stream so the ripple and any shatter look the same on every client watching the same landing
        ...
        // No per-landing one-shot. A field of individually triggered drops never resolved into rain: the trigger rate had to stay low enough to be affordable, which left it sounding like a handful
        // of taps rather than a downpour, and each sound fought the ambient bed instead of adding to it. The bed carries the rain now, and notifyImpact above is what makes it answer to how much is
        // actually landing where you are standing rather than to the weather parameters alone.
    }
}
...
void SSAtmoMagic::idle()
{
    ...
    // Parcel description polling and notecard fetch; must run before params are resolved so a config landing this frame is picked up immediately
    ...
    // Geometry changes that have held still long enough to be believed. Before the maps update, so a settled edit is picked up by this frame's capture.
    settleEdits();
    ...
    // What the weather is leaving on the surface. Reads the rain shadow map's captured geometry, never writes it.
    SSSurfaceField::getInstance()->idle(gFrameIntervalSeconds);

    // ...and on the people standing in it. After the surface field, which is what answers whether anything is over their heads.
    SSAvatarWet::getInstance()->idle(gFrameIntervalSeconds);

    // The volumetric cloud layer, rebuilt from the active track's field state. Drawn by the sky pass; this is only the field behind it. Strikes: scheduling, the leader/return-stroke phases, and
    // handing each clap to the soundscape at the moment it fires rather than the moment it is heard. Before the cloud field's update, which reads the strikes to light its puffs from inside - after
    // it, the deck's flicker would run a frame behind the bolt, and at return-stroke timescales a frame is most of a stroke.
    ...
    // Ambient rain/wind loops and the indoor probe cycle; fades everything out itself when the system is off
...
// How long an object has to stay put before either map will rebuild for it. Long enough that a thrown prim, a bullet or a combat rez is gone before its entry matures; short enough that editing a
// roof shows up while you are still looking at it.
static const F64 EDIT_SETTLE_SECONDS = 4.0;

// A rezzing region can produce updates faster than they settle, and it holds thousands of prims, so this has to be roomy: an entry is a few dozen bytes and the cap exists to bound a pathological
// case, not to ration normal use. When it is hit the entry *furthest* from maturing is the one dropped. This was the other way round once, and dropping the oldest meant that under heavy rezzing -
// exactly when the maps most need to hear about the new geometry - the entries about to be confirmed were the ones thrown away, so nothing ever matured and the shadow map never learned the buildings
// were there. Rain fell straight through their roofs until the periodic full recapture caught up.
static const size_t MAX_PENDING_EDITS = 4096;

// Confirmations per frame. Each one dirties two maps; doing thousands in the frame a big build finishes settling would be its own hitch.
static const size_t MAX_SETTLE_PER_FRAME = 64;

// How far an object may drift and still count as the same resting object. Interpolation and terse updates jitter a stationary prim by centimetres.
...
void SSAtmoMagic::onObjectUpdate(LLViewerObject* objectp)
{
    ...
    // Already tagged as a bullet by its shape: long, thin, and almost always gone within the second. Nothing shaped like that shelters anything from rain or blocks any wind worth solving for.
    ...
    // Note what is moving, do not discard it. Something in flight is not part of the build yet, but it may be about to become part of it - a prim being dragged into place, a physical object settling
    // - and an object whose last update carried a velocity would otherwise never be queued at all, so the maps would never hear about it. The settle pass re-checks position and keeps deferring until
    // it actually stops; a projectile is gone before that happens and is dropped without ever costing a rebuild.
    ...
    if (it != self->mPendingEdits.end())
    {
        ...
        // Moved again, so it has not settled. Restart its clock rather than letting a slow drag mature partway through.
...
void SSAtmoMagic::settleEdits()
{
    ...
    for (auto it = mPendingEdits.begin(); it != mPendingEdits.end(); )
    {
        ...
        {
            // Rezzed, did whatever it was doing, and went. This is the whole point of the delay: a projectile never gets this far, so it never costs a rebuild.
        ...
        {
            // Still moving, just not sending updates fast enough to have reset the clock. Give it another window.
            it->second.mPos = pos;
        ...
        // The rest keep until next frame. They are already past their settle time, so nothing is lost by finishing them a frame later.
...
// A beacon over every entry still waiting out its settle delay. The counts in the info overlay say the queue is busy but not what is keeping it busy, and the two causes look identical from a number:
// a stream of short-lived objects passing through costs nothing, while a few objects that never hold still are a permanent tax on both maps. This puts each entry where you can walk to it and see
// what it actually is.
void SSAtmoMagic::renderDebug()
{
    ...
    // How tall the beacons stand. Tall enough to clear the roof of whatever is holding the entry, since the offender is usually a prim inside a build rather than something out in the open.
    ...
    // The box is depth tested against the world, so a marker inside a wall reads as inside it. The beacon above it is not: its whole job is to be findable from across the region.
    {
        ...
        for (const auto& entry : mPendingEdits)
        {
            ...
            // Green means it is about to mature and cost one rebuild, which is the queue working. Amber means it has pushed itself back a few times. Red means it has been re-arming for long enough
            // that it is never going to settle on its own - a swaying tree, a hovering vehicle, a scripted prim nudging itself - and that is what this overlay exists to find.
            LLColor4 color = colorForEdit(edit);

            const LLVector3& p = edit.mPos;
            const F32 r = llmax(edit.mRadius, 0.25f);

            // Axis-aligned box at the size the maps were told to dirty, not at the object's own bounds: an entry that dirties far more than the thing causing it is worth seeing as such.
...

    {
        ...
        for (const auto& entry : mPendingEdits)
        {
            ...
            // The column fades out with height so a screen full of them still reads as a set of points on the ground rather than as bars
            const LLVector3& p = edit.mPos;
            const S32 steps = 6;
...
// Shared by the boxes, the beacons and the info overlay's offender list, so the colour in the readout is the colour standing over the object. static
LLColor4 SSAtmoMagic::colorForEdit(const PendingEdit& edit)
{
    SSAtmoMagic* self = getInstance();

    // Fully bright as it comes due, dimmer while it still has time to run, so a marker brightening and vanishing is a normal edit being believed
...
void SSAtmoMagic::drawInfo()
{
    ...
    // Feeds both maps below, so it sits ahead of them rather than inside either. A prim change is held here until it has stayed put long enough to be worth rebuilding for; "settling" high with
    // "believed" flat means neither map is hearing about the build at all.
    ...
    // A queue that sits at a steady handful is either harmless churn or the same few objects re-arming forever, and the count alone cannot tell you which. List the ones that have pushed themselves
    // back the most, with where to go and look; the beacon overlay marks these same entries in world in the same colours.
    {
    ...
    {
        // Age and build count together: the map is meant to be static, so a count that climbs while you stand still means something is churning
        ...
        // The travelling gust at this spot, and how long a wave takes to reach here from the region's windward edge. Watch the multiplier and you are watching the surge arrive.
        ...
        // What the mask holds over this exact spot. A column with nothing captured over it, or a volume that is barely solid anywhere, both mean the air has nothing to flow around however many
        // passes it gets.
        F32 top = 0.f;
    ...
    // What the rain shadow answers for the same column. Most land here is mesh and prims, so the difference between a surface the capture saw and the terrain heightmap it falls back to is the
    // difference between rain landing on the roof over you and rain starting inside the room.
    {
        LLVector3 hit;
        bool on_water = false;
    ...
    // How hard the map is working. "heightmap guess" above with a capture age climbing past the refresh interval means it has not caught up with the build; a forced count that never moves while a
    // region rezzes around you means it is not being told the build changed at all.
    {
    ...
    {
        // Peaks rather than means: a mean over a whole region is mostly the open ground nothing has happened to, and reads as zero long after the streets have gone dark.
    ...
    if (audio->isCovered())
    {
        // Ceiling from the up ray, and how much more build the flowmap's height capture says is stacked on top of it. A cellar reads as a couple of metres of ceiling with storeys of burial above it.
    ...
    // Footsteps: the last lookup for you and the last for anyone else, held rather than sampled live because a step is an event - by the time anyone reads this the foot has long since landed. Yours
    // gets the full readout; it is the one being debugged.
