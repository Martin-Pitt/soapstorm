/**
 * @file ssatmov3legacybridge.h
 * @brief Atmo Magic v3 -> v2 renderer bridge. SSAtmoMagic::refreshParams()
 *        is the existing, working rain/wind/particle driver - it is being
 *        kept, not replaced (see doc/atmo_magic_v3_environment.md's
...
#ifndef SS_ATMOV3LEGACYBRIDGE_H
#define SS_ATMOV3LEGACYBRIDGE_H

// <SS:Nexii> Atmo Magic v3 -> v2 renderer bridge (temporary, see file header)
...
class SSAtmoV3LegacyBridge
{
public:
    // False (out_cfg untouched) if there is no v3 asset loaded at all - the
    // caller should fall through to the existing SSAtmoTrackMgr path
    ...
    // a preset swap - see SSAtmoV3TrackResolver's neighbour weight).
    //
    ...
    // preset name is: SSPrecipPresetMgr::find() with the existing "empty or
    ...
    // number (whatever SSAtmoTrackMgr::currentTrack() happens to report)
