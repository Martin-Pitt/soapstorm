...
namespace
{
    // Beyond this an avatar is not tracked at all. Wetness is a specular
    // change on a body a few pixels tall at that distance; the cost of
    // carrying everyone in the region for it is not repaid.
    const F32 TRACK_RADIUS = 96.f;

    // How long an avatar keeps its state after we stop seeing them. Long
    // enough to cover a teleport-and-return or a walk behind a building,
    // short enough that a busy sim does not accumulate strangers forever.
    const F64 FORGET_SECONDS = 120.0;

    // Seconds of standing in the full downpour to go from dry to soaked, and
    // seconds of standing out of it to dry off again. Drying is deliberately
    // much slower: a soaked coat does not stop being wet the moment the cloud
    // passes, which is the same reason the puddles drain on their own clock.
    const F32 SOAK_SECONDS = 45.f;
    const F32 DRY_SECONDS  = 240.f;

    // How quickly the exposure figure itself moves. Walking through a doorway
    // should read as stepping out of the rain, not as a switch: without this
    // an avatar crossing a lip flickers between sheltered and not on the cell
    // boundary underneath them.
    const F32 EXPOSURE_TAU = 1.5f;

    // Cover clearance: a stored surface this far above someone's head is a
    // roof, and one below it is the ground they are standing on. Between the
    // two the exposure fades rather than stepping, which is what makes the
    // edge of an awning read as an edge rather than a line.
...
F32 SSAvatarWet::exposureAt(const LLVector3& foot_agent, F32 height) const
{
    ...
    // Nothing captured here: no evidence of cover, and open sky is the
    // commoner case by far. Assuming shelter instead would leave avatars
    // bone dry in the middle of a field whenever the capture had not caught
    // up with them.
    if (!sample.mValid) return 1.f;

    // The field stores the top of the column - the thing the weather lands
    // on. If that is above someone's head, it is what is keeping the rain
    // off them; if it is at their feet, it is the ground they are standing
    // on and the sky is open.
...
void SSAvatarWet::idle(F32 dt)
{
    ...
    // Only liquid water wets anybody. Snow settles on someone rather than
    // soaking them, and the preset knows which of the two is falling - the
    // same mWetRate the surface field asks about.
    ...
    // Distance-sorted as they are gathered, so the cap below keeps the
    // nearest rather than whichever the character list happened to hold
    // first.
    ...
    for (LLCharacter* character : LLCharacter::sInstances)
    {
        ...
        // Control avatars are animesh objects rather than people. They can
        // have wetness too eventually; leaving them out for now keeps the
        // capsule budget for the bodies anyone is looking at.
        ...
        // The body as a capsule: from the soles up. mBodySize is the
        // appearance's own measurement, so a tiny avatar gets a tiny capsule
        // and a nine-foot one gets a nine-foot capsule without this having
        // to guess.
        ...
        // Rate rather than target carries the intensity: a drizzle should
        // eventually soak someone who stands in it all day, just far more
        // slowly than a downpour, and scaling the target instead would cap
        // them permanently damp.
        ...
        cap.mFootAgent = foot;
        cap.mHeight = height;
        // Wide enough to hold clothing, hair and a rigged coat, and no wider:
        // this is a world-space test, so anything else standing inside the
        // capsule shades as though it were part of them.
    ...
    // Forget avatars nobody has seen for a while. Done here rather than on a
    // separate timer because this is the only place that knows who was seen.
...
bool SSAvatarWet::bindForShader(LLGLSLShader& shader) const
{
    ...
    // Two vec4s each: where the body stands and how wet it is. Packed rather
    // than sent as separate arrays so the whole set is two uploads however
    // many avatars are in it.
