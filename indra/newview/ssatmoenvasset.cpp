...
LLSD SSAtmoEnvWeather::asLLSD() const
{
    ...
    // The override values serialise even while Auto is on - authored
    // keyframes should survive a save made with Auto re-enabled
    // mid-experiment, not silently drop out of the notecard the way they
    // did when these were plain scalars only written on !auto.
...
bool SSAtmoEnvWeather::fromLLSD(const LLSD& sd)
{
    ...
    // The keyframe container reads a bare scalar as a plain value, so a
    // notecard written back when these were plain F32s parses through the
    // same call - see SSAtmoEnvKeyframed::fromLLSD's fallthrough.
...
bool SSAtmoEnvCelestialBody::fromLLSD(const LLSD& sd)
{
    ...
    // Absent on anything written before auto-naming existed - false (still
    // auto-named) is the right reading for those, no migration needed.
    ...
    // Absent in a notecard written while tilt was doing latitude's job, and
    // that is what it meant then - so an old document keeps the sky it had.
    mLatitudeDeg = llclamp(sd.has("latitude_deg") ? (F32)sd["latitude_deg"].asReal()
                                                 : mAxialTiltDeg, -90.f, 90.f);

    // Absent in a notecard written before these were authorable, where the
    // behaviour was derived from kind - so that is what absent means here,
    // and such a document loads looking exactly as it did.
...
S32 SSAtmoEnvPlanetary::addBody(SSAtmoEnvCelestialBody::EKind kind, S32 preferred_parent_index)
{
    ...
    // The canonical topology has no slot for a fifth star - the floater's
    // Add Sun button disables at the cap, this is the backstop behind it.
    ...
    // Shading defaults follow the kind at creation, and are authored from
    // there - a sun lights itself, everything else is lit by one. See
    // SSAtmoEnvCelestialBody::mEmissive.
    body.mEmissive = (kind == SSAtmoEnvCelestialBody::SUN);
    body.mPhaseShaded = !body.mEmissive;

    // "Moon 3": only ever seen on a body auto-naming skips (a moon whose
    // parenting fell through to orphaned) - everything else gets its real
    // name from autoNameBodies() below before this call returns.
    S32 same_kind = 0;
    for (const SSAtmoEnvCelestialBody& b : mBodies)
    {
    ...
    // Parenting is automatic - there is no parent dropdown any more. Suns
    // are normalizeSunTopology()'s business below. A planet always parents
    // to the first sun: for one sun that is literally it, for two the
    // resolver's barycenter rule anchors the planet at the pair's
    // barycenter, and for the 4-sun case "barycenter of ALL suns" would be
    // more honest but the inner pair's barycenter is an acceptable
    // approximation until someone actually needs better. A moon parents to
    // the caller's preferred planet (the floater passes the selection, or
    // the selection's own planet) when that names one, else the first
    // planet as a last resort - the floater disables Add Moon when no
    // planet exists at all.
    body.mParentIndex = -1;
    ...
    // Per-kind physical defaults on the same Sol/Earth/Luna scale
    // makeDefault() uses, so a freshly added body reads as the thing its
    // kind says rather than as the struct's one-size PLANET default. Mass
    // is in per-level units (solar/Earth masses - see the header), so each
    // default reads as 1.0-ish rather than some cross-level ratio. A sun's
    // orbital radius starts 0 - roots sit at the anchor, and
    // normalizeSunTopology() fills in a pair separation where one applies.
    //
    // A planet or moon arrives one step OUTSIDE its outermost sibling
    // rather than at a fixed radius - a fixed default would stack every
    // new body onto whichever sibling happens to sit there. The steps are
    // the familiar yardsticks (1 AU, one Luna distance), so the first of
    // each still reads as Earth's orbit and the Moon's.
    switch (kind)
    {
        ...
        case SSAtmoEnvCelestialBody::PLANET:
        {
            ...
            // All planets orbit the one sun anchor, so every planet is a
    ...
    // Creation-time lighting defaults - see the header. A fresh binary
    // should light the scene with both stars, and a lone-sun world gaining
    // a moon of the home planet gets that moon as its night light -
    // matching the renderer's sun+moon light slots. Defaults only: the
    // user can uncheck either afterwards and nothing re-asserts them.
    // canSetLightEmitter() is the never-steal guard (cap of two, home
    // excluded).
...
bool SSAtmoEnvPlanetary::removeBody(S32 index)
{
    ...
    // Removing a planet takes its moons with it - see the header for why
    // cascade replaced orphaning once re-parenting left the UI. The doomed
    // set is gathered up front so one old->new index map can fix every
    // survivor in a single pass, whatever got removed.
    ...
    // Old index -> new index, -1 for the removed. Survivor pointers into
    // the doomed set become -1 (a bound partner reverting to -1 is just
    // "not paired" - no special case); pointers above removals shift by
    // however many doomed bodies sat below them.
    //
    // Trace, removing the Planet from [Sun(parent -1), Planet(parent 0),
    // Moon(parent 1)]: Planet and Moon are both doomed, the Sun survives
    ...
    // The fixup above can leave a surviving pair member an isolated star
    // (its partner just went to -1) - re-normalising restores the one
    // canonical shape for however many suns remain. Planets need nothing:
    // their lineage is derived (effectiveParent) and their position
    // anchors at the sun group's barycenter, so there is no stored planet
    // parent to go stale. Names follow: the ordinals encode ordering, and
...
{
    // Suns in mBodies order - "structure order", the order they were
    // added, which is also the order auto-naming letters them in.
    std::vector<S32> suns;
    for (size_t i = 0; i < mBodies.size(); ++i)
    {
    ...
    // Dissolve every sun's existing pairing first (symmetrically, so a
    // partner reference from some other body can't dangle), then assign
    // the one canonical shape per star count. "One pair orbiting a giant
    // sun" needs no case of its own here - it is expressible as pair
    // separations/masses that put the barycenter inside the primary.
    //
    // Trace, adding four suns one at a time (indices = add order):
    //   1 sun:  0(parent -1, partner -1)
    //   2 suns: 0(-1, 1)  1(-1, 0)            - a bound root pair
    //   3 suns: 0(-1, 1)  1(-1, 0)  2(0, -1)  - 2 orbits the pair's
    //           barycenter via the resolver's partner-substitution rule
    //   4 suns: 0(-1, 1)  1(-1, 0)  2(0, 3)  3(0, 2) - the outer pair
    //           orbits the inner pair's barycenter
    // Then removing sun 1: survivors shift to 0,1,2 and re-normalise to
    //   0(-1, 1)  1(-1, 0)  2(0, -1)
    // - the old third sun is promoted into the root pair rather than
    // leaving the old first sun an isolated star.
    ...
    if (suns.size() >= 2)
    {
        ...
        // A sun promoted into an orbiting role with no authored separation
        // yet (radius 0, the root default) gets one; an authored radius is
        // the user's and survives re-normalisation.
...
namespace
{
    // Up to XXXIX, far past the body counts a system diagram stays legible
    // at - the subtractive pairs keep it exact rather than "IIII".
...
void SSAtmoEnvPlanetary::autoNameBodies()
{
    ...
    // The first sun's name is the stem everything below derives from -
    // renamed first so a fresh system's planets read "Sol I" rather than
    // deriving from a stale name. A custom-named first sun stems its
    // whole system ("Alpha B", "Alpha II"); no sun at all falls back to
    // the plain "Sol" stem rather than leaving planets unnamed.
    ...
    // Further suns letter B/C/D in structure order - the same order
    // normalizeSunTopology() builds pairs in, so the letters read
    // inner-pair-then-outer-pair by construction.
    ...
    // Planets take roman ordinals by orbital radius ascending across ALL
    // planets - they all orbit the sun anchor, so one distance ordering is
    // the ordering. Stable, so equal radii keep add order.
    ...
    // Moons: "<planet name>.N" by radius around their planet, using the
    // planet's name as it now stands - an auto "Sol I" or a custom
    // "Tatooine" both carry their moons ("Tatooine.1"). A moon whose
    // parent isn't a planet (orphaned by a removal) keeps its current
    // name; the list already marks it "(orphan)".
...
bool SSAtmoEnvPlanetary::setBoundPartner(S32 a, S32 b)
{
    ...
    // Only ever between same-parent siblings - see mBoundPartnerIndex. Two
    // roots (both -1) count as siblings, which is exactly the binary-star
    // case the design doc names.
    if (mBodies[a].mParentIndex != mBodies[b].mParentIndex) return false;

    // Dissolve whatever either one was in first, so a partner being
    // reassigned can never leave a third body still pointing at it -
    // symmetry is the invariant the resolver relies on.
    clearBoundPartner(a);
    clearBoundPartner(b);
...
bool SSAtmoEnvPlanetary::clearBoundPartner(S32 index)
{
    ...
    // The partner's back-reference only clears if it actually points here -
    // a hand-edited notecard can break symmetry, and stomping some other
    // pairing it names would compound the damage rather than contain it.
...
bool SSAtmoEnvCloudField::fromLLSD(const LLSD& sd)
{
    ...
    // The keyframe container reads a bare scalar as a plain value, so a
    // notecard written back when these were plain F32s parses through the
    // same call - see SSAtmoEnvKeyframed::fromLLSD's fallthrough. The old
    // >= 0 clamps live in SSAtmoEnvCloudFieldResolver now - see the header.
...
namespace
{
    // addKeyframesFromSky's per-field stamp. setValueAtHead alone would
    // write the PLAIN value while the field has no keyframes yet (that is
    // the widget editing rule it implements), and a plain value overwritten
    // per stamp can never accumulate into a cycle - so promote the field
    // first: toggleKeyframeAtHead turns the value in force at `phase` into
    // a real keyframe, which setValueAtHead then finds and edits.
    template <typename T>
    ...
    // Tolerance collapseConstantKeyframes() hands collapseIfConstant().
    // Skies that agree on a field hand over byte-identical F32s (settings
    // assets store them verbatim), so this only has to absorb float noise -
    // and it must NOT be coarse enough to merge genuinely different small
    // values, since density multiplier legitimately lives around 1e-4.
    // Ids and strings compare exactly regardless - see ss_atmoenv_near_equal.
...
{
    // Whole-container assignment on purpose, for the same reason the
    // atmosphere's own seeding does it - see
    // SSAtmoEnvAtmosphere::fromSettingsSky below.
    ...
    // The sky's packed density/detail colours unfold into the three
    // scalars EEP's own panel edits - see the header on why the schema
    ...
    // The stock noise texture reads back as null: the schema's "null =
    // default" convention should hold from the document's first save
    // rather than pinning today's default id into the notecard.
...
{
    // Field set and conversions match fromSettingsSky above exactly - the
    // packed density/detail unfold, the null-for-stock noise map - only the
    // write differs: a keyframe stamped at `phase`, leaving whatever is
    // authored at other phases alone.
...
bool SSAtmoEnvCloudDome::fromLLSD(const LLSD& sd)
{
    ...
    // Absent keys keep their constructor defaults, same as the water and
    // atmosphere structs - a hand-written notecard that only sets coverage
    // gets LLSettingsSky's stock cirrus layer for everything else.
...
{
    // Whole-container assignment on purpose: each field becomes a plain
    // value with no keyframes, which is the only state a freshly created
    // asset can be in anyway - this is a seeding step, not a merge.
    ...
    // The RAW authored ambiance, not the RenderSkyAutoAdjustLegacy-aware
    // read EEP's own refresh uses - that flag is a per-viewer display
    // convenience, and a transcription should carry what the sky says.
    ...
    // Nothing else is read on purpose. Sun/moon rotations, disc textures
    // and scales are the Planetary tab's to compute from bodies (see the
    // struct comment), and every cloud-layer field the sky carries -
    // colour included - is SSAtmoEnvCloudDome::fromSettingsSky's to read,
    // seeded alongside this call.
}

void SSAtmoEnvAtmosphere::addKeyframesFromSky(const LLSettingsSky& sky, F64 phase)
{
    // Field set and conversions match fromSettingsSky above exactly - see
    // there for the raw-ambiance read, the glow packing maths and why
    // nothing positional is read. Only the write differs: a keyframe
...
LLSD SSAtmoEnvAtmosphere::asLLSD() const
{
    ...
    // Same keys LLSettingsSky itself uses for these four, so a notecard
    // and a settings asset read alike where they overlap.
...
bool SSAtmoEnvAtmosphere::fromLLSD(const LLSD& sd)
{
    ...
    // This key used to round-trip as an opaque LLSD blob "pending its own
    // phase", and nothing ever wrote structured content into it - every
    // saved notecard carries it as an empty map. So there is no migration
    // to do and no version bump to take: reading the known keys and
    // ignoring anything else covers every asset that exists.
    //
    // Absent keys keep their constructor defaults, same as the water
    // struct - a hand-written notecard that only sets a haze value gets
    // LLSettingsSky's stock sky for everything else, not black.
...
LLSD SSAtmoEnvWeatherInfluence::asLLSD() const
{
    ...
    // Written flat rather than as a map per mapping: six pairs is not
    // enough structure to be worth the nesting, and flat keys keep a
    // hand-edited notecard readable.
...
bool SSAtmoEnvWeatherInfluence::fromLLSD(const LLSD& sd)
{
    ...
    // Every field keeps its constructed default when absent, so a notecard
    // written before this struct existed loads as "influence on, everything
    // at full strength" - which is what a fresh environment gets too, so an
    // older document and a new one behave alike rather than diverging on
    // the strength of their own weather.
...
SSAtmoEnvAsset SSAtmoEnvAsset::makeDefault()
{
    ...
    // orbiting a 1-solar-mass sun. The sun is flagged as the one light
    // emitter so a freshly created environment already has *something*
    // lighting it rather than a black sky by default. The planet's radius
    // is exactly 1 AU so the floater's AU display reads a clean 1.00, and
    // masses are the per-level units' own 1.0 (solar/Earth masses - see
    // the header). Both names are autoNameBodies()'s ("Sol", "Sol I"),
    // with mNameCustom left false so they keep tracking any restructuring.
    ...
    // Authored disc art rather than EEP's built-in sun, which is a
    // null texture id meaning "let the renderer draw its own" - a real
    // texture is what lets the same body be drawn as a billboard when it is
    // not the one in the sun slot.
    sun.mCustomTexture = LLUUID(SSAtmoEnvCloudDome::BODY_TEXTURE_SUN);
    ground.mPlanetary.mBodies.push_back(sun);

    // Earth, transcribed rather than approximated - the default world is
    // the one every author starts from, so it may as well be the one they
    // already know the sky of. Real values also make the derived machinery
    // honest: 23.44 degrees of tilt is what puts the celestial pole where
    // a temperate observer expects it, and the rise/set markers on the
    // editor's scrubber then read as a real day rather than an equinox at
    // the equator.
    ...
    // Earth's orbit IS the ecliptic, so its inclination is zero by
    // definition in the usual frame - the number worth carrying is its
    // 7.155 degrees against the Sun's own equator, which is the plane
    // bodies here orbit in. (At phase 0 an inclination has no effect at
    // all - the orbit crosses the reference plane there - so this is
    // carried for correctness rather than for anything it does today.)
    home.mOrbitalInclinationDeg = 7.155f;
    // Phase 0, which in THIS model means equinox, not "the start of the
    // year". A home body's orbital phase does not behave like a
    // heliocentric longitude here: the reference plane contains the
    // rotation axis at zero tilt (see celestialAxis), so phase is really
    // the angle between the sun and the celestial equator - a season dial.
    // Phase 0 puts the sun ON the equator, rising due east and culminating
    // at 90 - tilt = 66.6 degrees, which is exactly an equinox at a
    // temperate latitude and the most useful place to start authoring
    // from. Real Earth's J2000 mean longitude of 100.46 degrees would put
    // the sun 19 degrees from the celestial pole instead, which in this
    // convention is polar night: a default world where the sun never rises
    // at all.
    home.mOrbitalPhaseDeg = 0.f;
    home.mIsHome = true;
    ground.mPlanetary.mBodies.push_back(home);

    // ...and its moon, likewise real, and likewise a light emitter: EEP's
    // moon slot goes to the smaller of the two emitters (see
    // resolveLightRoles), so Luna lights the night while Sol keeps the sun
    // slot, and a fresh environment has both lights an author expects to
    // find rather than a sun over an empty night sky.
    ...
    // Thirty degrees off the planet's own orbital phase.
    //
    // Matching it exactly puts the moon on the far side of the planet from
    // the sun - a full moon, which sounds ideal and is in fact a permanent
    // total lunar eclipse: dead on the anti-sun line, inside the planet's
    // shadow, every hour of every day. Real Luna escapes that because its
    // orbit is inclined and the nodes rarely line up; this model is static,
    // so it has to be placed off the line deliberately.
    //
    // Thirty degrees is a waxing gibbous - (1 + cos 30) / 2, so about 93%
    // lit, still reading as "a full moon" - while putting it around 190,000
    // km off the shadow axis, which is thirty times the planet's own radius
    // and nowhere near the umbra.
