...
// <SS:Nexii> The horizon clip (SSAtmoEnvAtmosphere::mHorizonClip), sampled at the applied phase
// like the mirror - the same uniform skyF.glsl reads for the depth write and the optics cut. 1
// while the authored sky asks for the hard waterline: the lower dome draws (to take the disc
// with it), and the sun's glow and haze effects stop at the horizon line - see the fade below.
...
// below it; 0 also means no active Atmo environment, which is exactly stock. The dome's light and
// its sun glow are ramped on this below. Stock switches BOTH the moment the disc's centre crosses
// zero - and the first cut of the ramp ran it across the disc's own span, which scaled the glow
// down by the risen share of the disc and halved the sunset exactly where the authored skies put
// it at full strength. So the band runs DOWN from the horizon instead: the disc's light hits the
// atmosphere long before the disc reaches the horizon and keeps lighting it long after, and the
// ramp now honours both - full glow from centre-rise to centre-set, the dusk easing out below.
...
// both alive through the whole band, dusk included, and the takeover rule is sun > moon until
// the band is spent: the glow and the extinction below keep aiming at the SUN while
...
// unit the horizon band is sized in. The extinction below holds the sun's airmass at the
// JUST-CLEARED path (off_axis 1/radius at the horizon line) while the rise band is live - through
// the whole rise AND the whole dusk below the horizon - because elevation 0 is not the
// horizon-sitting airmass - it is the infinite one, and 1/max(1e-6, rel_pos.y + 0) collapses every
// horizon ray to unlit. The floor releases continuously once the centre climbs past it, and the
...
// channel. 2.0 is the warm-orange horizon band of a real sunset - deep enough to burn, shallow
// enough to stay bright; raise it toward the darker reds, lower it toward the golds. See the
// long note at the glow light below.
const float SS_SUN_GLOW_DEPTH = 2.0;

// <SS:Nexii> How far below the horizon line the sun's glow and haze effects fade out over when
// the clip is on, as a SINE of the angle below eye level - about a degree - so the cut reads the
// same from any camera height. See the fade at vary_ss_below_horizon below.
...
void main()
{
    ...
    // <SS:Nexii> Under the clip the sun's glow and haze effects stop at the horizon line. The
    // clip already takes the disc and the weather optics with it (skyF.glsl's depth write and
    // ss_clipped_below), but the glow's true-ray hotspot and its capped light would otherwise
    // carry on down past the waterline - the natural continuation the mirror was built to
    // provide. An authored sky that asks for the clip asked for the hard waterline instead, so
    // the directional glow term fades out over SS_SUN_HORIZON_FADE below the line (sine of the
    // angle below eye level - scale-free, the same cut from any height) and only the mirrored
    // AIR - the body terms - continues below. With the clip off the fade stays 1 and the glow
    ...
    //
    // The mirror is for the AIR terms only - extinction and haze body, which
    // have no direction of their own. The directional terms (the glow's
    // hotspot, the rainbow and halo strip, the weather optics) scatter from
    // the light at a TRUE angle, so they read the ray captured here before
    // the flip: feeding them the mirrored ray painted a second copy of the
    // sun's glow and corona below the horizon, the mirror image of the one
    // above it.
#ifdef SS_ATMO
    ...
    // <SS:Nexii> Under the mirror the rainbow and halo strip read the true ray (see above), so
    // their rings land at true angular distances from the light below the horizon too - never
    // mirrored ones.
    ...
    // direction uniform and the same +Y up the dome already shades with. Under the mirror this is
    // the TRUE ray (see above): a below-horizon fragment gets only what its real angle from the
    // light scatters - the natural continuation of the phenomena across the horizon line - never
    // the mirrored copy the flipped ray used to paint.
...
#ifdef SS_ATMO
    // <SS:Nexii> The disc sheds light across the horizon band, not the instant its centre clears
    // the horizon: the dome's sunlight is the full day colour from centre-rise up - the condition
    // the authored skies painted against - and walks back down to the night value as the sun
    // sinks through the twilight below the horizon (ss_sun_rise is the horizon-band share).
    ...
    // DISC'S OWN half-angle (ss_sun_radius). Stock feeds the raw (clamped) lightnorm elevation in
    // here, so the moment the disc's CENTRE dips under, every ray near the horizon loses its
    // light path - the max(1e-6, ...) below collapses them to unlit - and the whole sunset band
    // is cut out from under a disc that is still half up. Holding the elevation at the radius is
    // the JUST-CLEARED airmass (1/radius at the horizon line, the same path a sun that has only
    // just cleared gets), so the band stays lit for the whole rise, and the dusk below the
    // horizon keeps that same just-cleared path while the band's share fades it out - the
    // afterglow dies of the fade, never of the airmass. The floor releases continuously once the
    // centre clears the radius, and the gate is stock with it off.
    ...
    // whole time the disc is up, which is exactly the stock sun line and the condition the
    // authored skies painted against, then easing out across the twilight below the horizon.
    // Stock's factor line cannot be allowed to touch it in the band: below centre-rise the
    // factor belongs to the moon (< 1.0), whose branch zeroes the term entirely (SL-13768 -
    // right for the moon, which must not glow), and ramping on the zeroed term grew a FLAT 0.25
    // wash with no hotspot at all until the factor snapped to 1.0 at centre-rise - the sunrise
    // horizon simply was not there while the disc poked over. With the gate off, stock.
    if (ss_sun_rise > 0.0)
    {
...
#ifdef SS_ATMO
    // <SS:Nexii> The horizon clip's cut of the sun's effects at the waterline - see the note at
    // ss_glow_horizon_fade above. 1 unless the clip is on and this vertex sits below the line.
    haze_glow *= ss_glow_horizon_fade;
#endif

#ifdef SS_ATMO
    // <SS:Nexii> The sun's glow and haze get their OWN light, with the beam extinction's crush
    // BOUNDED rather than the beam's. The beam maths above (off_axis 1/elevation) hands horizon
    // rays an effectively infinite airmass, and the density product exp()s the sun's colour to
    // black there - the glow's SHAPE survives but arrives colourless, which is the grey horizon
    // band, the "edge" the authored sunset is supposed to burn through. (A radial fade of the
    // extinction around the disc was tried first and read as a giant ball: the window's inner
    // zone rode the hotspot at full strength, the mid-ring swept through the deepest saturation,
    // and the window's edge popped back to the crushed sky - a bullseye around the sun quad,
    // which is itself correct.) So instead the glow's extinction is CAPPED: its airmass may not
    // exceed the depth at which the densest attenuation channel has shed SS_SUN_GLOW_DEPTH
    // optical depths - scaled uniformly across the channels, so the hue survives and the band
    // deepens TO its colour, never through it and out the other side. Where the beam maths is
    // mild the cap never binds and the stock line stands; it only binds where the crush was
    // total, and a min() has no edge to see - the grey band simply becomes the bright warm
    // gradient a real sunset carries, core to horizon and on down past it. Density keeps every
    // other duty untouched - the dome body, the blue_horizon wash, the distance haze, the
    // (1 - combined_haze) scale. Live only while the rise band is (ss_sun_rise > 0): an idle
    // environment keeps the stock line bit for bit.
