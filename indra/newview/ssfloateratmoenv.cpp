...
// U+25C6 filled / U+25C7 hollow, as escaped UTF-8 bytes rather than literal glyphs - that keeps this file pure ASCII, so a tool that mis-guesses the encoding cannot mangle them. One already did:
// they had become C3A2 C297 C286, which is E2 97 86 decoded as latin-1 and re-encoded.
static const char* const FILLED_DIAMOND = "\xE2\x97\x86";
static const char* const HOLLOW_DIAMOND = "\xE2\x97\x87";

// Rise/set markers on the preview scrubber. Same escaped-UTF-8 treatment as the diamonds above, and the same Geometric Shapes block they come from: U+25B2/U+25BC are the full-size up/down triangles,
// U+25B4/U+25BE their small siblings. Big = the sun, small = the moon, so which body a marker belongs to reads at a glance without a legend, in the same way the sun is the bigger light in the sky.
...
// Warm daylight amber against a pale moonlit blue-white - the two lights' own colours, kept dim enough that markers annotate the scrubber rather than competing with the keyframe ghosts drawn on the
// same strip.
...
// How far outside the preview scrubber still counts as hovering it, for the whole-tab keyframe overview. The scrubber is an 18px strip in a busy floater; without a little slack around it the
// overview flickers on and off as the pointer drifts a pixel or two.
...
bool SSFloaterAtmoEnv::postBuild()
{
    ...
    // The rail's name buttons are the tab selector for the panel on the right - slot N is mTracks[N], see refreshAltitudeLabel().
    ...
    // Commit only - deliberately no mouse-down hook. LLMultiSlider fires its mouse-down signal *before* it works out which thumb was actually hit (see LLMultiSlider::handleMouseDown), so a handler
    // there reads the previously selected thumb and picks the wrong track. Selection belongs to the name buttons anyway; dragging a thumb selects that track too, but from the commit, where
    // mCurSlider is correct.
    ...
    // The Weather Influence editor is reachable from every tab whose parameters weather can modulate; all four buttons open the one floater. findChild rather than getChild: a tab whose panel failed
    ...
    // Water tab - the one non-keyframed control; everything else on that tab is a keyframe row set up alongside the Weather cube's below.
    getChild<LLUICtrl>("water_enabled_check")->setCommitCallback(
        [this](LLUICtrl*, const LLSD&) { onCommitWaterEnabled(); });

    // The Auto structural toggles - see refreshAutoRows() for what flipping one greys out and what fills the greyed rows in.
    ...
    // mSelectedTrackIndex's Weather cube, one AE-style row per field - see the FloatRow comment in the header. mField captures `this` rather than being cached, so switching the selected track (or an
    // add/remove track reallocating SSAtmoEnvAsset::mTracks) can never leave a dangling reference or a stale track index behind.
    mFloatRows = {
    ...
    // The Water tab's own scalar rows - same mechanics, same widget naming convention, just a different struct on the far end of the accessor.
    ...
    // The Clouds tab's Volumetric Field sub-tab rows - the storm layer's artist baseline. Live coverage/density derive from these plus the weather cube (see ssatmoenvcloudfieldstate); the legacy
    // Windlight cloud plane (cirrus) is the Sky Dome sub-tab's below, a separate thing these rows never touch.
    ...
    // The Sky Dome sub-tab's scalar rows - the legacy cirrus layer's EEP parameter set (see SSAtmoEnvCloudDome). The non-scalar rows (colour, scroll vector, noise texture) join their typed lists
    // below.
    ...
    // The Atmosphere & Lighting tab's scalar rows - the EEP haze/gamma/glow set (see SSAtmoEnvAtmosphere). No azimuth/elevation rows anywhere here, deliberately: sun/moon position is computed from
    // the Planetary tab's home body, not keyframed on this tab. The colour half of the tab joins mColorRows below.
    ...
    // The water height slider's ceiling comes from the constant rather than from the XML alone, so the bound the floater enforces and the one the schema documents cannot drift apart. It is only a UI
    // bound - see SS_ATMOENV_WATER_CEILING - so a value loaded from a notecard above it is displayed pinned at the top of the slider rather than rewritten.
    ...
    // The Water tab's non-scalar keyframed controls. Split by type rather than crammed into one list - see the KeyRow comment in the header. The HDR scales EEP's own editor uses - see
    // KeyRow::mScale.
    ...
    mColorRows = {
        ...
        // Atmosphere & Lighting's colour half - same swatch row the water fog colour uses, one entry per EEP colour. Cloud colour is the Sky Dome's row below, not this tab's - it moved out with the
    ...
    // The dome's noise picker previews the stock cloud noise for a null value and allows None - null IS the schema's "default noise" value (see SSAtmoEnvCloudDome::mNoiseTexture), so without an
    // explicit None choice an author could never step a keyframe back to the default. Same pair of calls EEP's own clouds panel makes on its picker.
    ...
    // The Weather tab's precipitation override - a dropdown over a keyframed string. Same cluster machinery as every other row; the string's own lerp is what makes its keyframes hold rather than
    // blend.
    mStringRows = {
...
{
    // Give the real wall clock back control the moment nobody's actually looking at a preview - see SSAtmoEnvManager::setPreviewPhaseOverride(). onVisibilityChange (below) already covers the common
    // single-instance hide-not-destroy case, but this catches actual destruction too.
...
void SSFloaterAtmoEnv::onVisibilityChange(bool new_visibility)
{
    ...
    if (new_visibility)
    {
        // Re-establish the override immediately rather than waiting for draw()'s next 0.5s poll tick, so reopening the floater doesn't leave the live scene on the real wall clock for a moment first.
        refreshPreview();
    }
    else
    {
...
void SSFloaterAtmoEnv::reshape(S32 width, S32 height, bool called_from_parent)
{
    ...
    // After the base class has moved everything the layout owns, put the rail's own markers back where the rail now says they belong. See the header on why the periodic refresh cannot cover this.
...
void SSFloaterAtmoEnv::draw()
{
    ...
    if (now - mLastPoll > STATUS_POLL_INTERVAL)
    {
        mLastPoll = now;
        refreshStatus();
        // Whether hasAsset() has flipped since the last check - a fetch completing while this floater sits open should reveal the editing chrome without needing to be reopened. Track list and
        // keyframe proof are refreshed here too rather than only at click time, since loadFromInventory()'s fetch is asynchronous and may complete well after the click that started it.
        refreshVisibility();
        refreshTrackTab();
        // Skip the keyframe/slider refresh (including the altitude rail's own full rebuild) while any of this floater's own widgets has mouse capture (a slider mid-drag) - forcing every slider's
        // thumb back to its last-committed value out from under an active drag, every 0.5s, made a real drag look like it was fighting itself or "not sticking". A drag's own hover handler already
        // keeps mPreviewPhase/the field values/the dragged track's altitude live via its continuous commit, so skipping this one poll tick loses nothing.
    ...
    // After the children, so the ghosts sit on top of the scrubber they annotate rather than being painted over by it. Rise/set markers go down first: they are permanent furniture, and a keyframe
    // ghost landing on the same pixel is the more urgent of the two to read.
    drawRiseSetMarkers();
    drawKeyframeGhosts();
...
{
    // Type dispatch: a notecard drop loads a whole environment (below, unchanged); a settings drop imports one EEP sky into the loaded one, so it needs an asset to import into - with nothing loaded
    // there is no selected track to stamp, and the drop is refused at hover time.
    ...
    *accept = ACCEPT_YES_SINGLE;
    ...
    if (drop)
    {
        ...
        const bool started = SSAtmoEnvManager::getInstance()->loadFromInventory(item,
            [this](bool success)
            {
                // This callback only ever fires for a load the floater itself asked for (a drag-and-drop drop) - parcel discovery's own fetches go through a completely separate path
                // (SSAtmoEnvDiscoveryManager) that never sets one, so this alert can never fire for an unattended background load.
                if (!success)
                {
        ...
        if (started)
        {
            // The fetch is async; the callback above (and the regular poll in draw()) pick up the eventual result either way - this just reflects that a load attempt is now in flight.
            refreshStatus();
        }
        else
        {
...
{
    // Re-read through the model rather than trusting the drag's raw pointer - same guard LLSettingsDropTarget applies to its own DAD_SETTINGS drops.
    const LLViewerInventoryItem* item =
        drop_item ? gInventory.getItem(drop_item->getUUID()) : nullptr;
    if (!item || item->getAssetUUID().isNull()) return;

    // FULL-PERM GATE: import means transcribing someone's settings asset into a plain notecard the author can then hand around, so only an item the owner holds copy + modify + transfer on may come
    // in - the same PERM_ITEM_UNRESTRICTED check the texture-save and copy-asset-id gates use (see LLViewerInventoryItem::checkPermissionsSet).
    ...
    // Capture what the author saw at drop time: the track they had selected and where the preview head sat. The fetch below is async, so resolving against live members instead would stamp wherever
    // the author happened to have scrubbed to by the time it lands.
    ...
    LLSettingsVOBase::getSettingsAsset(item->getAssetUUID(),
        ...
        {
            // The floater may have closed while the fetch was in flight - a dead handle means nobody is editing any more, so the drop just evaporates.
            ...
            if (!sky)
            {
                // A water or day-cycle settings item. Water would be a trivial follow-up - SSAtmoEnvWater mirrors EEP's water panel one-for-one, it just needs its own addKeyframesFromWater
                // transcription - while a day cycle is a bigger lift (each of its frames would expand into a stamp of its own), so both are refused for now.
                LLNotificationsUtil::add("GenericAlert", LLSD().with(
                    "MESSAGE", "Only sky settings can be imported for now."));
                return;
            }

            // The loaded asset may have been replaced or reshaped (tracks removed) while the fetch was in flight - re-read through the manager and re-check the captured index rather than assuming
            ...
            // Dropping a sky stamps its look as keyframes at the preview head: the author scrubs to a time, drops a sky, and that instant now looks like that sky - repeatable at other phases to
            // build a whole day out of EEP presets. Both structs stamp because a sky's fields split across them, exactly as in creation-time seeding.
...
void SSFloaterAtmoEnv::refreshVisibility()
{
    ...
    // track_panel and atmo_tabs now own every per-track/per-field widget as descendants (the vertical track list and the Weather tab's rows respectively), so hiding those two containers is enough -
    // no need to walk every row individually the way this did back when they were all direct floater children. load_button stays out of this list, same as before - it works with nothing loaded yet,
...
void SSFloaterAtmoEnv::refreshTrackRail()
{
    ...
    // Selection can't outlive a track being removed out from under it (Remove Track always drops back to the ground track), so clamp before it drives anything below.
    if (mSelectedTrackIndex >= (S32)asset.mTracks.size())
    {
        mSelectedTrackIndex = 0;
    }

    // Cleared and rebuilt from the asset every call, same as LLPanelEnvironmentInfo::refresh() does for its own altitude sliders - simpler than diffing which thumbs already exist, and cheap enough
    // at this many. Never called mid-drag - see the draw() and onCommitAltitudeSlider() comments.
    ...
    for (S32 slot = 1; slot <= SS_ATMOENV_MAX_TRACKS - 1; ++slot)
    {
        ...
        // addSlider refuses a value too close to an existing marker (see overlap_threshold on the rail): a notecard authored before that minimum existed, or hand-edited, can ask for two tracks
        // almost on top of each other. Walk upward a step at a time until one sticks rather than silently ending up with no marker for the track - the same nudge LLPanelEnvironmentInfo::refresh()
        // does for out-of-range altitudes from the server.
        ...
        // Whatever the rail actually accepted is the truth from here on, so write it back rather than leaving the asset claiming an altitude no marker is sitting at.
...
void SSFloaterAtmoEnv::repositionRailMarkers()
{
    ...
    // Ground is pinned to the rail's own 0m position rather than being a draggable thumb - it is 0m by definition. The rail runs all the way down to 0 so ground sits on the same scale as every other
    // track rather than reading as a footnote below it, and it is placed by the same arithmetic a real marker is, so "where 0m is" is answered once. Its XML follows="left|bottom" would otherwise
    // park it a fixed distance above the panel's bottom edge, which is only where 0m happens to be at one particular floater height.
    ...
    // ...and each optional track's own labels, which are likewise placed against the thumb rather than anchored.
...
S32 SSFloaterAtmoEnv::railCentreForValue(F32 value) const
{
    ...
    // A thumb's centre travels between thumb_width/2 and height - thumb_width/2; this is LLMultiSlider::setSliderValue()'s arithmetic verbatim, S32 truncation included, so a marker placed by this
    // lands on exactly the pixel the rail drew its thumb on. thumb_width is read back off a real thumb rather than hard-coded, so nothing here has to be kept in step with the XML by hand. Any thumb
    // will do - they are all the same size - and the fallback only matters before the first one exists (a lone Ground track).
    S32 thumb = 8;
    for (S32 slot = 1; slot < SS_ATMOENV_MAX_TRACKS; ++slot)
    {
...
void SSFloaterAtmoEnv::refreshAltitudeLabel(S32 slot)
{
    ...
    // Same placement routine the Ground marker uses, so the two agree by construction rather than by two hand-matched offsets.
    const S32 centre = railCentreForValue(value);
    centreViewOn(name_button, centre);
    centreViewOn(alt_label, centre);

    // Name on the left of the rail, altitude on the right - the same arrangement the region Environment tab uses for "Sky 3 / 2000m". The name is whatever the author set (see
    // SSAtmoEnvAsset::nextDefaultTrackName), so it comes from the asset rather than being formatted from the index here.
    const SSAtmoEnvAsset& asset = mgr->asset();
...
void SSFloaterAtmoEnv::onCommitAltitudeSlider()
{
    ...
    // Fires continuously while dragging, so this does the least it can get away with: write the value and move that one marker's own labels. Emphatically no reordering or rail rebuild here -
    // rebuilding calls addSlider(), which sets LLMultiSlider's mCurSlider to whatever it added last, so doing it from this callback silently transferred the in-progress drag to the topmost marker.
    // That work happens on mouse-up instead, see onMouseUpAltitudeSlider(). The rail runs down to 0 so ground reads as part of the same scale, but ground owns everything below the first track - so a
    // marker dragged into that band is pushed back out here rather than the rail being given a floor of its own. Writing it back to the control is what makes the thumb actually stop, instead of the
    // value being silently corrected while the thumb keeps following the cursor.
    ...
    // Actually moving a track is the one case where touching the rail also selects: you are unambiguously working on that track by then, and mCurSlider is reliable here in a way it isn't at
    // mouse-down time (see postBuild).
    ...
    refreshAltitudeLabel(slot);
    // Moving one track changes which band its neighbours cover, which the Track tab spells out.
    refreshTrackTab();
    refreshStatus();
}
...
void SSFloaterAtmoEnv::onMouseUpAltitudeSlider()
{
    ...
    // Safe to reorder now the drag is over - see the header comment. If the marker was dragged past a neighbour the array order changed with it, so follow the selection through and rebuild the rail
    // from scratch.
    ...
    // The sort can have renumbered every track; an open designer must follow the same track to its new index rather than silently editing whichever one now sits at the old number.
...
{
    // One floater, opened from four places - see floater_ss_atmo_weather_influence.xml on why the mappings are not split across the tabs they affect. Keyed on the selected track, the same way the
    // System Designer is.
...
void SSFloaterAtmoEnv::selectTrack(S32 index)
{
    ...
    // An open System Designer follows the selection - each track is its own planetary system, and the designer editing a track this floater is no longer looking at would be two views silently
    // disagreeing.
    SSFloaterAtmoPlanetary* designer =
        LLFloaterReg::findTypedInstance<SSFloaterAtmoPlanetary>("ss_atmo_planetary");
    if (designer)
    ...
    // ...and so does the Weather Influence editor, for the same reason: influence is per track, like the weather cube it reads.
    SSFloaterAtmoInfluence* influence =
        LLFloaterReg::findTypedInstance<SSFloaterAtmoInfluence>("ss_atmo_influence");
    if (influence)
    {
...
{
    // Same underlying write as the inventory "New Settings > New Atmo Magic" menu item (see menu_create_inventory_item's "atmoenv" branch); the only difference is this also adopts the result once it
    // lands, so opening the floater with nothing active never dead-ends. Deliberately does not re-fetch the notecard we just wrote to load it back - gInventory.getItem()/loadFromInventory() would
    // work, but only once local inventory has actually caught up with the new item, which is one more race to fall into for no reason. Nor does it regenerate SSAtmoEnvAsset::makeDefault() here: that
    // used to be safe because makeDefault() was deterministic, but creation now seeds the atmosphere from a fetched Midday sky (see createDefaultNotecard), so only the asset the callback hands back
    // is guaranteed to be the content that was actually written.
...
{
    // There is no generic inventory item picker to call, so this opens inventory and reveals the "Atmo Magic" folder under Settings (see SSAtmoEnvManager::atmoFolderId) rather than filtering the
    // whole panel to notecards - a type filter left the panel's own filter indicator reading "All Types" while actually restricting what showed, which read as broken rather than filtered. Everything
    // Atmo Magic creates lives in that one folder, so revealing it directly needs no filter at all. Dropping a notecard onto this floater does the actual load; this button is a shortcut to the right
    // place, not a picker.
...
void SSFloaterAtmoEnv::onClickUnload()
{
    ...
    // No confirmation prompt: nothing is destroyed. The notecard is still in inventory and the parcel still references it - this only stops this viewer showing it, and the applier hands EEP's local
    // slot back on the next frame because hasAsset() has gone false.
    mgr->unload();

    // The rail, the tabs and the preview all read the asset, so they have to be told it is gone rather than discovering it a poll later.
    mSelectedTrackIndex = 0;
    refreshVisibility();
    refreshTrackRail();
    refreshTrackTab();
...
void SSFloaterAtmoEnv::onClickAddTrack()
{
    ...
    // You just asked for a new track - you almost certainly want to look at it, not stay on whatever was selected before.
...
//-----------------------------------------------------------------------------
// Track tab - floor/ceiling/day-cycle/water for mSelectedTrackIndex. See panel_ss_atmo_env_track.xml.
//-----------------------------------------------------------------------------
...
void SSFloaterAtmoEnv::refreshTrackTab()
{
    ...
    // Left alone while it has focus, so a refresh landing mid-typing (the 0.5s poll in draw(), say) can't yank the caret or discard a partial edit - same rule the asset-level name field follows.
    ...
    // Water tab: the enable checkbox is the one control there that isn't a keyframe row, so it refreshes here with the rest of the per-track structural state rather than in refreshPreview().
...
void SSFloaterAtmoEnv::onCommitTrackName()
{
    ...
    // An unnamed track would leave a blank button on the rail with nothing to click on; fall back rather than allow that.
...
void SSFloaterAtmoEnv::onCommitDayCycle()
{
    ...
    // Slider and spinner are two views of one value; whichever the user just touched is the authority, and the refresh below puts the other in step.
    ...
    // Nothing to rescale. Keyframes and the preview head are both stored as a position in the cycle rather than a duration into it, so day length is purely how fast the cycle plays - it cannot move
    // a keyframe or shift what the scrubber is pointing at.
    refreshTrackTab();
    refreshStatus();
}
...
// The three structural lightning switches. One handler for all of them because they are read and written together and none of them means anything without the others - the charge and the sparks are
// effects AROUND a strike, so they are moot on a sky that has no lightning.
...
namespace
{
    // Defined with the other display helpers further down; forward declared so refreshAutoRows can share the same name mapping the combo items and ghost labels use.
...
void SSFloaterAtmoEnv::refreshAutoRows()
{
    ...
    // One evaluation feeds every computed readout below - the gust and lightning figures Auto derived, and the precipitation type - so they can never describe different instants.
    ...
    for (const auto& row : rows)
    {
        ...
        // The whole keyframe cluster too: a keyframe inserted into a field Auto is ignoring only parks confusion for later, and chevrons stepping the head between keyframes that visibly do nothing
        // read as broken rather than parked. This runs after the generic row refresh re-enabled them by keyframe count, so this verdict wins.
        ...
        if (row.mAuto)
        {
            // The greyed controls become a live readout of what Auto decided for this instant (refreshFloatRow already wrote the parked authored value moments ago; this deliberately overwrites it).
            // A disabled spinner cannot hold focus, so no focus guard is needed here.
            getChild<LLUICtrl>(prefix + "_slider")->setValue(row.mComputed);
            getChild<LLUICtrl>(prefix + "_value_spinner")->setValue(row.mComputed);
        }
    }

    // What Auto would drop at this instant, beside the combo - only while the combo actually says Auto; a forced type makes it redundant.
...
void SSFloaterAtmoEnv::refreshWaterRows()
{
    ...
    // Every water control is named "water_..." - the prefix IS the grouping, so this walks the row lists rather than naming thirty widgets and going stale the next time one is added.
    auto setRow = [this, enabled](const std::string& prefix, LLUICtrl* primary)
    {
        if (primary) primary->setEnabled(enabled);

        // The keyframe cluster too: a keyframe on a plane that does not exist is confusion parked for later, and chevrons stepping between keyframes that visibly do nothing read as broken.
...
bool SSFloaterAtmoEnv::rowAutoOwned(const std::string& prefix) const
{
    ...
    // Water with no plane is inactive in exactly the sense this function exists to describe: the field is real and keyframed, but nothing is reading it, so it neither edits nor ghosts.
...
//-----------------------------------------------------------------------------
// Planetary tab - the two per-track distance-scale dials and the button into the System Designer (SSFloaterAtmoPlanetary), which is where the celestial bodies themselves are edited now. See
...
void SSFloaterAtmoEnv::refreshPlanetaryScales()
{
    ...
    // Slider and spinner are two views of one value, spinner left alone while focused, same as every other slider/spinner pair.
...
void SSFloaterAtmoEnv::onCommitPlanetaryScales()
{
    ...
    // Whichever of the pair the user just touched is the authority - the same rule onCommitDayCycle() applies.
    const bool from_spinner =
...
void SSFloaterAtmoEnv::onClickOpenPlanetaryDesigner()
{
    ...
    // The designer's key is the track to edit; selectTrack() keeps an already-open instance following this floater's selection from there on.
    LLFloaterReg::showInstance("ss_atmo_planetary", LLSD(mSelectedTrackIndex));
}

//-----------------------------------------------------------------------------
// Keyframe rows - AE-style, one per animatable field across every tab. See ssatmoenvkeyframe.h for the container every row exercises, and the FloatRow comment in the header for why mField is a fresh
// accessor rather than a cached reference. refreshPreview() is the one entry point that re-reads everything driven by the preview phase; its name used to be refreshKeyframeProof, a fossil from when
// this was a one-slider proof of concept.
//-----------------------------------------------------------------------------

namespace
{
    // "Apparent Time of Day" - same idiom and the same [HH]:[MM][AP] (PRC%) template LLPanelEnvironmentInfo::udpateApparentTimeOfDay() uses for the region/estate Environment tab, so a position in
    // the cycle reads as a clock time rather than a raw fraction - and the "(41%)" is that same position stated plainly. Takes the phase directly: day length is a playback-speed control and has
    // nothing to say about what time of day a given point in the cycle represents.
    std::string formatApparentTime(F64 phase)
    {
        // Deliberately not wrapped: sitting exactly at the end of the loop is 24:00 (100%), the same instant as 0:00 (0%) but reached from the other side, and reporting it as 0% made dragging the
        // scrubber to the far right look like it had snapped back to the start.
        ...
        {
            // hour 24 only ever comes from the exact-end-of-cycle case above; it reads as midnight the same way 0 does.
    ...
    // Display names for the precipitation override's stored values, matching the combo's own item labels so a ghost keyframe and the combo read the same. Empty is the Auto entry; anything
    // unrecognised (a hand-edited notecard) shows its raw value rather than nothing.
    std::string precipDisplayName(const std::string& value)
    {
...
//-----------------------------------------------------------------------------
// Shared keyframe-cluster machinery. Every row type - scalar slider, colour swatch, texture picker, xy_vector - has the same diamond/prev/next behaviour and differs only in how its own control is
// read and written, so that half lives here once rather than four times.
//-----------------------------------------------------------------------------
                                               ...
                                               const SSAtmoEnvKeyframed<T>& field, F64 phase)
{
    ...
    // Chevrons are only meaningful once there is more than one keyframe to move between - with none or one, "jump to the next" has nowhere to go.
...
{
    // refreshPreview() (called by every caller of this) is what actually re-reads every control from mPreviewPhase - one place that converts time to what the controls show, not one per row.
...
// Whether a colour picker is open anywhere. The picker is a separate floater that a swatch owns through a protected handle, so a swatch cannot be asked directly. Walking the floater view for a
// visible one answers the same question and is not fussy about which swatch opened it - which is what we want, since any open picker is one somebody is typing into.
...
void SSFloaterAtmoEnv::refreshColorRow(const KeyRow<LLColor3>& row, F64 phase)
{
    ...
    // Divided into the swatch's own 0..1 space - see KeyRow::mScale. The stored value is EEP's, which for several of these runs past 1.
    const F32 inv = (row.mScale > 0.f) ? (1.f / row.mScale) : 1.f;
    const LLColor3 value = field.valueAt(phase) * inv;

    // The swatch is an LLColor4 control; none of the keyframed colours has a meaningful alpha of its own, so each round-trips through a fully opaque colour and only the RGB is ever stored. Skipped
    // entirely while a colour picker is open. The periodic refresh would otherwise write the swatch every half second, the picker would resync its own fields from it, and a hex code being typed in
    // would be replaced mid-keystroke - which is exactly what it looked like.
...
void SSFloaterAtmoEnv::commitColorRow(const KeyRow<LLColor3>& row)
{
    ...
    // ...and multiplied back out of it on the way in, so what is stored is always EEP's own value whatever the swatch had to show.
...
void SSFloaterAtmoEnv::refreshVectorRow(const KeyRow<LLVector2>& row, F64 phase)
{
    ...
    // xy_vector's own LLSD form is a two-element array of reals, which is exactly how ss_atmoenv_value_to_sd<LLVector2> stores it - so this is a straight handover with no reshaping.
    ...
    // The graphic's own X/Y entry bar is collapsed (edit_bar_height="0"): at a parameter row's height there is no room for both it and a usable touch area, and its two boxes would not line up with
    // any other row's number anyway. The numbers live in spinners on the left in the same column as every other row's, and the graphic is purely the direct-manipulation half.
...
void SSFloaterAtmoEnv::refreshStringRow(const KeyRow<std::string>& row, F64 phase)
{
    ...
    // A value outside the combo's vocabulary (a hand-edited notecard, or one from a build with a wider list) displays as Auto rather than leaving the combo on whatever it showed last. Only the
    // display: the stored value is untouched until the user actually commits something.
...
void SSFloaterAtmoEnv::refreshFloatRow(const FloatRow& row, F64 phase)
{
    ...
    // Left alone while it has focus so a refresh landing mid-typing cannot yank the caret or discard a half-entered number - the same rule the name fields follow.
...
void SSFloaterAtmoEnv::commitFloatRow(const FloatRow& row)
{
    ...
    // This one call is the entire editing rule from the design doc: no keyframes yet, this just becomes the plain value; head on an existing keyframe edits it in place; head anywhere else inserts a
    // new one.
...
void SSFloaterAtmoEnv::refreshPreview()
{
    ...
    // This floater being open with a preview visible is what overrides the live render's clock - see SSAtmoEnvManager::setPreviewPhaseOverride() and SSAtmoEnvBridge::resolveActiveTrack(). Every call
    // here (a user scrub, a chevron jump, or just the periodic poll while nothing's changed) re-asserts it, so the in-world preview always matches whatever this floater is currently showing rather
    // than drifting back to the real wall clock the instant this stops being re-armed.
    mgr->setPreviewPhaseOverride(mPreviewPhase);

    const F64 day_length = mgr->asset().mTracks[mSelectedTrackIndex].mDayLengthSeconds;

    // The scrubber works in phase, 0..1, the same unit keyframes are stored in - so changing a track's day length rescales nothing here and moves nothing. It still snaps to the design doc's 32 slots
    // across the cycle; that grid is a fraction of the cycle too, not a duration. The scrubber is a PERCENTAGE of the cycle, because that is what a phase is: the track's day length scales it into a
    // time of day afterwards (see formatApparentTime) and has no business deciding what the control's own range means. A scrubber calibrated in hours would have to be rebuilt whenever a track's day
    // length changed, and two tracks running different day lengths could not share one.
    ...
    // Phase 5 proof: the same resolve() call phase 5's renderer hookup will eventually feed the particle/cloud/audio systems from.
    ...
    // Last, so the Auto groups can overwrite the authored values the row loops above just displayed - see refreshAutoRows() - and so the water tab's greying survives the row loops having just
    // re-enabled every keyframe cluster by keyframe count.
    refreshAutoRows();
    refreshWaterRows();
}
                                   ...
                                   FormatFn format, std::vector<GhostKeyframe>& out)
{
    ...
    for (S32 i = 0; i < count; ++i)
    {
        ...
        {
            // First keyframe's span reaches back to the start of the cycle and the last one's runs to the end, rather than either wrapping around: a marker that wrapped would be drawn at the far end
            // of the scrubber from the keyframe it belongs to, which reads as a different keyframe entirely. Everything in between runs to its successor, so the spans partition the cycle with no
...
{
    // The keyframe button is the one widget every row type has regardless of what its actual control is, so its vertical extent stands in for the row's band. Horizontally the whole tab counts:
    // pointing at a row's label or its slider is pointing at that row.
    LLView* tabs = findChild<LLView>("atmo_tabs");
    LLView* button = findChild<LLView>(prefix + "_keyframe_button");
    if (!tabs || !button) return false;

    // Must be on the tab that is actually showing. getVisible() only reports the widget's own flag, so a row on a hidden tab still passes it - and since every tab uses the same row grid, the Water
    // row at a given height would answer for the Weather row at that same height. isInVisibleChain() is the question that was actually meant.
...
bool SSFloaterAtmoEnv::scrubberHovered() const
{
    ...
    // Deliberately a padded copy of the rect rather than a taller widget. Growing the slider itself would also grow what it captures on a click-drag, so aiming near it would start scrubbing rather
    // than just revealing the overview - and it would push the layout below it around. Only the hover test wants the extra room, so only the hover test gets it.
...
bool SSFloaterAtmoEnv::collectHoveredKeyframes(std::vector<GhostKeyframe>& out, bool& out_labels) const
{
    ...
    // Hovering the scrubber itself asks a different question - "where is there anything keyframed on this tab at all" - so it gathers every animatable parameter currently on screen rather than one
    // row. Rows belonging to a tab that isn't showing are skipped: their keyframes are real but they are not what is being looked at.
    ...
    for (const FloatRow& row : mFloatRows)
    {
        ...
        // Formatted exactly as this row's own readout is, so a ghost and the live value can be compared without mentally converting between two notations.
    ...
    {
        if (!wanted(row.mPrefix)) continue;
        // Same scale the swatch shows, so a ghost and the control under it read the same number - this is where <765, 765, 765> was coming from for a plain white sunlight.
...
bool SSFloaterAtmoEnv::scrubberGeometry(LLRect& out_rect, S32& out_left_edge, S32& out_travel) const
{
    ...
    // The scrubber is a direct child of the floater, so its rect is already in the space these overlays draw in (LLFloater::draw has left the origin at the floater's own bottom-left).
    out_rect = scrubber->getRect();

    // Match LLSlider::updateThumbRect()'s travel exactly, so an overlay sits on the pixel the thumb would occupy at that phase rather than merely near it. The thumb image is the one named by
    // slider_bar.xml; its width is the inset at each end.
...
void SSFloaterAtmoEnv::drawRiseSetMarkers()
{
    ...
    // The same two bodies the renderer will light the world with - asked of the resolver rather than picked here, so a marker can never annotate a different "sun" than the one that actually comes
    // up.
    ...
    // On the track line itself, HCENTER/VCENTER exactly like the ghost diamonds - the ghosts' value labels live in the lanes above and below, so the two overlays share the strip without colliding.
                    ...
                    const char* rise_glyph, const char* set_glyph)
    {
        ...
        F64 rise = 0.0;
        F64 set = 0.0;
        // A body that never crosses the horizon draws nothing rather than being pinned to an edge: on a world where the sun never sets there is no sunrise to point at, and a marker parked at phase 0
    ...
    // Moon first: where the two coincide (a body rising as the other sets, or a tight double), the sun's larger glyph reads through the smaller one rather than the other way round.
...
void SSFloaterAtmoEnv::drawKeyframeGhosts()
{
    ...
    // Value labels alternate above and below the scrubber when they would otherwise collide. Keyframes cluster - a dawn ramp is three of them inside a few percent of the cycle - and overlapping
    // labels there are worse than useless, since two half-drawn numbers read as neither. Greedy, left to right: take the upper lane if it is clear, otherwise the lower one, otherwise whichever has
    // been free longest. That last case only arises when labels are too dense for two lanes to hold, where something has to overlap regardless; picking the emptier lane at least keeps the damage to
    // one pair.
    ...
    for (const GhostKeyframe& ghost : ghosts)
    {
        ...
        // Filled marks the keyframe currently in force, the same filled/hollow distinction the row's own diamond button uses. For a HOLD keyframe that means the head being anywhere in its span, not
        // just on its boundary - the whole point of drawing it as a span is that the value is in force across all of it.
...
void SSFloaterAtmoEnv::drawSliderValueGhosts()
{
    ...
    // Only while the scrubber is hovered. Left on permanently these compete with the thumb for attention on every row at once, and the thumb is the thing you are usually reading. Tying them to the
    // same gesture that raises the scrubber overview also pairs the two halves: one gesture, both axes - when each parameter has keyframes, and what values they hold.
    if (!scrubberHovered()) return;

    // Widgets on a tab are positioned relative to that tab, but drawing here happens in the floater's own space (LLFloater::draw has left the origin at its bottom-left). Screen rects are the common
    // ground: the difference between a widget's and the floater's gives the widget in floater-local coordinates without walking the view hierarchy.
    ...
    for (const FloatRow& row : mFloatRows)
    {
        ...
        // Same travel LLSlider::updateThumbRect() uses, so a ghost lands on the pixel the thumb would occupy at that value.
        ...
        for (const SSAtmoEnvKeyframe<F32>& kf : field.keyframes())
        {
            ...
            // Filled marks the keyframe the head is actually on, matching the row's own diamond button and the scrubber ghosts. When it is filled it sits under the thumb by definition - that
...
void SSFloaterAtmoEnv::advancePreviewPlayback()
{
    ...
    // Unloading the asset takes the scrubber with it, so the head has nothing left to run along.
    if (!SSAtmoEnvManager::getInstance()->hasAsset())
    {
        onClickPreviewPlay();
        return;
    ...
    // One lap a minute, matching Edit Day Cycle. Deliberately not the track's authored day length: that is hours, and the point of the button is to see the whole cycle rather than to sit through it.
    ...
    // A pause - the floater unfocused, a texture load stalling the frame - would otherwise jump the head across a chunk of the cycle it never showed. Better to lose the time than to skip the sky.
    if (elapsed <= 0.0 || elapsed > 1.0) return;

    mPreviewPhase += elapsed / PLAY_LAP_SECONDS;
    while (mPreviewPhase >= 1.0) mPreviewPhase -= 1.0;

    // Straight to refreshPreview rather than through the slider: this is the same head move the scrubber makes, and routing it through the control would fight the poll below that writes the control
    // FROM the phase.
    refreshPreview();
}
...
{
    // Dragging the scrubber takes over - a head that carried on running out from under the drag would be unusable.
    ...
    // Percent in, phase out - see refreshPreview for why the control is calibrated that way. The readout beside it is what turns the phase into a legible time of day.
