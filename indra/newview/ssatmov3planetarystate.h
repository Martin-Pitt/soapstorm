/**
 * @file ssatmov3planetarystate.h
 * @brief Atmo Magic v3: resolves a track's celestial bodies into fixed sky
 *        directions/angular sizes, and computes the primary sun's
 *        azimuth/elevation arc from the home body's spin - phase 6 of
 *        doc/atmo_magic_v3_environment.md. Pure logic only, same as the
...
#ifndef SS_ATMOV3PLANETARYSTATE_H
#define SS_ATMOV3PLANETARYSTATE_H
...
class SSAtmoV3PlanetaryResolver
{
public:
    ...
    // fixed/authored - see SSAtmoV3CelestialBody - there is no simulated
    ...
    static std::vector<LLVector3> resolveWorldPositions(const SSAtmoV3Planetary& planetary);
    ...
    static std::vector<SSAtmoV3ResolvedBody> resolveSky(const SSAtmoV3Planetary& planetary);
    ...
    // by the caller - see SSAtmoV3Track::currentDayCycleTime()), not by any
...
#endif // SS_ATMOV3PLANETARYSTATE_H
