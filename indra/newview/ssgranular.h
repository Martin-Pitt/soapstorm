...
// singletons, LLSD - the extraction has collapsed back into the monolith it exists to avoid.
struct SSGranularParams
{
    ...
    F32 mDepositGap = 0.7f;    // deposit gate as a fraction of the lift threshold - the
                               // hysteresis that keeps a cell from eroding and banking at once
...
{
    // One fixed-step integration over one region. Settle/melt has already run in the field's own
    // tick; this is what the wind does to what settle just left.
    void step(SSSurfaceField::Field& fld, const SSSurfaceField::Geometry& geom,
              const SSGranularParams& p, F32 dt);

    // Landing credit for a runoff clump: repose-capped, overflow discarded (a clump landing on a
    // full pile barely changes it - see the architecture doc's sharp edges).
