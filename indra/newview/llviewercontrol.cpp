...
void settings_setup_listeners()
{
    ...
    // <SS:Nexii> The Atmo Magic master switch rebuilds shaders: stock sky/atmosphere shaders compile PRISTINE with it off and compile the SS_ATMO variants with it on, so every Atmo-era fix to
    // shared shading lives behind the toggle instead of quietly changing the vanilla viewer. Toggling pays one shader rebuild, which is what this listener class exists for.
