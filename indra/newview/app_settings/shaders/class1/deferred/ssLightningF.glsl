...
// Ribbons or blobs: the channel is a strip falling off across its width; the flash lighting the sky around a strike is a disc falling off from its centre. One shader, because they are the
// same additive glow with two shapes, and swapping programs mid-pass to say so would cost more than the branch.
...
void main()
{
    ...
    {
        // A hot filament inside a soft sheath. The fourth power makes the middle read as a discharge rather than a painted stripe - the falloff from a line source really is this steep.
