...
// How far the surface may move under a cell before what has accumulated there is taken to belong to something else. A prim nudged a few centimetres is the same roof; one rezzed a metre up is not.
static const F32 REBUILD_DZ      = 0.35f;

// Slope is read from the height field, and across the lip of a roof that gradient is not a slope at all - it is a cliff with open air on the far side. Neighbours further than this below a cell are
// left out of the fit, so the edge of a flat roof reads flat, which is what it is.
static const F32 SLOPE_STEP_MAX  = 3.f;     // multiples of the cell size


// The stitched window handed to the shaders. 256 cells at the drainage resolution is 256 metres around the camera on a standard region, which is further than a damp pavement reads at all, and a
// megabyte of texture.
static const S32 WINDOW_RES = 256;
static const F32 WINDOW_NO_SURFACE = -1.0e6f;   // texel the stitch found nothing for

// Weather that says nothing about drying must not freeze the world in what the last lot left on it. Rain gives way to embers, or to nothing at all, and the street still has to recover - slowly, but
// it has to get there. These are the rates used when the running preset offers none of its own, and they are the reason a preset can opt into marking the surface without also having to state how the
// marks come off.
...
// The resolution the geometry is reduced to. The same figure the drainage trace used, kept because it is what the Field lattice and the stitched window are both built around - the window copies
// cells rather than resampling them, which only works while everyone agrees on the cell size. The gradient at which a surface is running water as hard as this system draws it - about 40 degrees, a
// steep roof pitch. Anything steeper streaks at full strength rather than more; there is no more to give.
static const F32 SLOPE_RUN_FULL = 0.85f;

// How far a neighbour has to sit below a cell before the cell counts as a lip rather than a step. A kerb is not an eave; a roof edge over a two storey drop is.
static const F32 GEOM_EDGE_DROP = 1.5f;

// Shedding. A lip catches the rain landing on its own footprint plus an allowance for the surface feeding it from behind, and that allowance is inferred from the local pitch rather than routed: a
// steep roof delivers what lands on it to its eaves, a flat one mostly holds it. This is the deliberate simplification that replaced the drainage solve - the number is a plausible metres-squared,
// not an accounting of where water went.
...
// Drops that gather into one drip on the way down. Water arrives at an eave as fewer, fatter drips than the rain that fed it, which is also what keeps a warehouse roof from costing more particles
// than the rain landing on it.
static const F32 SHED_MERGE = 12.f;

// Where an edge stops shedding drops and starts running as a sheet, in drips per second, and how far past that it takes to become one entirely.
static const F32 SHED_STREAM_MIN  = 8.f;
static const F32 SHED_STREAM_FULL = 24.f;

// Ceilings, per lip cell and per frame. A downpour on a city block is thousands of lips; the look does not improve past a handful of runs near the camera, and the particle pool has ripples and
// splashes to serve too.
...
// Widest curtain a preset may ask for, so one stream cannot span a bridge. Carried over from the shedder this replaced, where it bounded the same preset field.
static const F32 STREAM_SPAN_MAX = 24.f;

static const S32 GEOM_RES = 128;

// Above this gradient a cell is a wall rather than a surface anything lies on. Water still runs down it (that is what the slope channel is for) but nothing settles or stands.
static const F32 GEOM_WALL_SLOPE = 1.2f;

// A cell holds standing water when it is flatter than this and no neighbour is meaningfully lower - "this is a dip", asked of the eight cells around it and nothing further away. Deliberately strict:
// a puddle is a dip in the ground, and the looser this gets the more a level plaza turns into one enormous sheet of standing water.
static const F32 GEOM_POOL_SLOPE = 0.06f;

// Height differences below this are the capture's own noise rather than relief. A level street reads as a field of sub-millimetre ties, and letting that decide which way a cell drains is how a
// puddle ends up somewhere different on every recapture.
...
void SSSurfaceField::buildGeometry(const SSRainShadowMap::SurfaceGrid& grid, Geometry& out)
{
    ...
    // How far apart two neighbouring cells' heights may be and still be the same surface. Beyond it they are a roof and the ground below it, and a gradient across that pair would describe a cliff
    // that no water follows - it would just be the lip.
    ...
    for (S32 y = 0; y < n; ++y)
    {
        for (S32 x = 0; x < n; ++x)
        {
            ...
            // Central differences where both neighbours are on this surface, one-sided where only one is, zero at a lip. The result is the direction water on this cell runs - straight down the
            // slope, which is all the directionality this system claims to model.
            ...
            // Standing water: flat enough, and a dip rather than a shelf. Both halves matter - "flat" alone would flood every rooftop and pavement, which is exactly the runaway the size limit is
            // meant to prevent.
    ...
    // Lips, in a second pass so it can read the slopes the first one wrote. A cell is a lip when a neighbour is open air or a long way below it: the edge of a roof, a balcony rail, a bridge deck.
    // Water reaching one comes off it, and that is all the "drainage" this system needs to know - no network, no catchment, no routing. Which way it comes off is the sum of the directions the drops
    // lie in.
    for (S32 y = 0; y < n; ++y)
    {
        for (S32 x = 0; x < n; ++x)
...
void SSSurfaceField::refreshGeometry()
{
    ...
    for (const auto& entry : tiles)
    {
        ...
        // Keyed on the geometry revision, not the capture: the tile is recaptured whenever the camera climbs out of its band or the wind swings the fall direction, and neither of those changes the
        // shape of a roof. Rebuilding on every capture would re-derive an identical answer several times a minute.
    ...
    // Regions the shadow map has forgotten (walked out of range, or torn down) take their geometry with them - the Field on top of it is evicted separately, on its own idle timer.
...
// How long a lip takes to shed what it is holding, in seconds. This is what keeps an eave running after the rain stops - and what keeps it running THROUGH a gust lull, which is the failure the
// reservoir exists to prevent: a shed rate taken straight off the instantaneous weather made every eave in the region stutter in time with the gusts.
static const F32 SHED_DRAIN_TAU = 6.f;

// Ceiling on the reservoir, as multiples of what one second of the current inflow adds. A roof holds a film, not a tank; without this a long storm would bank hours of water and go on shedding it
...
void SSSurfaceField::shedEdges(F32 dt)
{
    ...
    // Drops per square metre per second the weather is actually delivering. Sampled once at the camera rather than per lip: the field it comes from drifts over hundreds of metres, so it says the
    // same thing anywhere in the shedding radius.
                                ...
                                F32 dt, F32 rate_m2, const LLVector3& camera_agent)
{
    ...
    // Drips share the effects pool with ripples and splash crowns, and in heavy rain the splashes alone can fill it - budgeting eaves against the whole pool shut them off exactly when it was raining
    // hardest, which is precisely backwards. They are budgeted against their own share instead.
    ...
    // Every lip integrates its reservoir every frame, wherever it is: a roof fills and drains whether or not anyone is watching it, and gating the accounting on distance would mean walking up to a
    // building in a downpour and finding its eaves dry for the first few seconds. Only the shedding itself - the particles - is near-camera work, and only a slice of the lips are even considered for
    // it in any one frame.
    ...
    for (S32 k = 0; k < lip_count; ++k)
    {
        ...
        // The catchment that is left: this cell's own footprint, plus an allowance for the pitch behind it. Statically inferred - a steep roof delivers what lands on it to its edge, a flat one
        // mostly holds it - rather than accumulated through a flow network.
        ...
        // Nothing left to shed, and nothing arriving: skip the rest, which is all particle work.
        if (outflow <= 0.01f)
        {
            fld.mAccum[ui] = 0.f;
            continue;
        ...
        // Only a slice of the lips gets as far as spawning anything in any one frame - see SHED_VISIT_PER_FRAME. The window starts at the rolling cursor and wraps, so over a few frames every lip
        // gets its turn and none is starved.
        ...
        // Where what comes off this lip lands. Resolved now rather than stored: only the handful of lips actually shedding this frame ask, and the answer changes whenever anything is built below
        // them.
        ...
        // Past a point an edge stops shedding drops at all: a gutter in a downpour comes off as a continuous fall, and drawing that as more and more separate drips both reads wrong and costs a
        // particle each.
        const F32 stream_drive = llclamp(
            (raw_rate - SHED_STREAM_MIN) / SHED_STREAM_FULL, 0.f, 1.f);

        // One stream per lip cell, keyed stably on where it is, so the same edge is the same stream frame to frame and client to client.
        ...
        if (stream_drive > 0.f)
        {
            // The curtain spans this cell's own width by default, so a run of neighbouring lip cells puts up neighbouring curtains that meet end to end - a gutter, without anything having to know
            // that the run exists. A preset asking for a particular span is taken at its word instead, the same way the old shedder took it.
        ...
        // Drips are the spatter around a running stream, not a second helping of it: once the stream is up it carries the water, and a full share of drips on top reads as the roof shedding twice.
        ...
        {
            // Anywhere across the cell's width rather than exactly on its centre: a metre-wide lip shedding down one line reads as a dotted seam, which is the one thing a roof edge never looks like.
    ...
    // Advance by what was actually looked at, not by the window size: a region whose lips are all out of range would otherwise spin the cursor through them at a frame each for no reason.
...
SSSurfaceField::Field* SSSurfaceField::fieldFor(U64 region_handle, const Geometry& geom, F64 now)
{
    ...
    // A resolution change is a different grid, not a moved one, and there is nothing sensible to carry across it
    if (fld.mN != geom.mN)
    {
        fld.mN = geom.mN;
        ...
        // Nothing has been standing in the weather yet, but the surface has to start out agreeing with the geometry or the first tick would read every cell as freshly rebuilt and reset what it just
        // cleared
        fld.mZ = geom.mZ;
    }
                          ...
                          const SSPrecipPreset& preset, F32 intensity)
{
    ...
    // Repose as a plain angle rather than a cosine: the preset says degrees, and a taper that reaches zero exactly where the preset says it should is worth more here than saving an atan per cell.
    const F32 repose = llclamp(preset.mSnowRepose, 5.f, 89.f) * DEG_TO_RAD;

    // Whether what is falling is the kind of thing that does each of these, rather than merely whether anything is falling. Rain over lying snow is not a snowfall that happens to have a rate of zero
    // - it is a thaw, and the melt below is what has to run.
    ...
    // A ceiling on top of whatever a preset authors, independent of it, because nothing here actually models a body of standing water yet - no shared level, no volume, no spilling into the next
    // hollow over once one fills. A single deep hollow with a whole roof draining into it will happily be told to stand as deep as the preset's own figure lets it, and until there is a real
    // basin-fill simulation to make that mean something, an unbounded depth just means an increasingly wrong-looking one. This is meant to come out again once that simulation exists.
    ...
    // Where puddles are ALLOWED, as a smooth patch mask over the region. The pool test above is geometric - "does this cell lie in a dip" - and over the flat ground most regions are mostly made of,
    // everywhere passes it at once. That is what made standing water arrive as square fields of shine butted against the hard channels of the rain shadow, instead of as the scattered pools the word
    // puddle means. The mask is value noise over the cell lattice, thresholded softly, so water stands in patches with dry ground between them and shallow rims where a patch fades out. Seeded from
    // the cell coordinates alone: region-local, so the pattern holds still across sessions and region crossings, and every viewer watching the same rain sees the same pools.
    ...
    for (S32 y = 0; y < n; ++y)
    {
        for (S32 x = 0; x < n; ++x)
        {
            ...
            // Open air. Nothing to dress, and leaving stale values in the cell would have them reappear if geometry came back under it.
            if (!geom.solid(i))
            {
            ...
            // Open water is not a surface the weather marks. It has its own shading and its own response to rain, and a wet, snowed-over lake would be neither.
            if (geom.water(i))
            {
                fld.mWet[i] = fld.mSnow[i] = fld.mPuddle[i] = 0.f;
                continue;
            ...
            // How readily the cell lies flat enough to hold anything, from whichever neighbours are on the same surface. A cell at the lip of a roof has open air on one side and the ground five
            // metres down beyond it; folding that in would call a flat roof a cliff and refuse to let snow settle on it. Only snow asks, so only snow pays for it. This is an atan and a square root
            // per cell, which over a region at four ticks a second is worth not doing on a wet night.
            auto lieHere = [&]()
            {
            ...
            // Wetness. The one value here that every surface takes, whatever the weather is doing to it otherwise - snow lying on a roof leaves the bare edges of it damp rather than dry.
            fld.mWet[i] += (wet_target - fld.mWet[i]) * wet_blend;

            // Settled snow, up to what the slope will hold - and no further: a pitch too steep to hold snow stays bare.
            if (snowing)
            {
                // Only what is arriving slides. Snow already lying does not start moving because the pitch it settled on was always that steep; it got there by falling more slowly than it slid.
                ...
                {
                    // What will not lie here simply does not settle. It used to be handed to the cell downhill, which was a water -movement model in all but name; a steep roof now just stays bare
                    // and the drift at the foot of it builds from the snow landing there, which is where it was coming from anyway.
                }
            }
            else if (fld.mSnow[i] > 0.f)
            {
                // Nothing is settling, so what is here is going: to a thaw, to the sun, or to the rain now falling on it
                fld.mSnow[i] = llmax(0.f, fld.mSnow[i] - snow_loss);
            }

            // Standing water, only in a cell the geometry says is a dip (see buildGeometry) and only up to the ceiling. There is no catchment term any more: a puddle is fed by the rain landing in
            // it, not by water routed to it from a whole roof face, so its depth is bounded by the weather rather than by how much upstream a solver decided to hand it. That bound is the point - an
            // unbounded catchment share is what let one hollow stand absurdly deep while the cell beside it stayed a film.
            if (pooling && geom.mPool[i])
            {
                // Gain and ceiling both wear the mask: a partially-masked cell fills slowly AND stays shallow, which is what gives a pool its thin fading rim instead of a cliff edge.
            ...
            {
                // Either the weather has stopped delivering, or a rebuild has changed its mind about this cell being a dip at all
...
void SSSurfaceField::idle(F32 dt)
{
    ...
    // A preset that never marks anything is not a reason to throw away what an earlier one left: the weather changing from rain to embers should let the street dry, not blink it dry. But with
    // nothing to integrate and nothing left to decay, there is no work to do.
    ...
    // Shape first, then what the weather has done to it: a region captured this frame is dressed this frame rather than next.
    refreshGeometry();
    ...
    // Eaves: what the roofs are holding, and what comes off them. After the tick, so a lip sheds against this step's weather rather than last step's.
    shedEdges(step);
...
// Stitch every field in range into one camera-centred grid and hand it to the GPU. Rebuilt whole on each tick rather than scrolled: at a megabyte four times a second that is a few megabytes a second
// of upload, against the bookkeeping of tracking which rows moved and which regions changed under them. If that ever matters, the window is snapped to whole cells precisely so it can be scrolled
// instead.
void SSSurfaceField::updateWindow()
{
    ...
    // Cell size comes from whichever field the camera is standing in, so the window and the drainage agree texel for cell and nothing is resampled
    const LLVector3 cam = LLViewerCamera::getInstance()->getOrigin();
    F32 cell = 0.f;
    {
    ...
    // Snapped to whole cells: a window that slid by fractions of a texel would put every cell somewhere new each tick, and the shading over a still scene would crawl
    ...
    for (const auto& entry : mFields)
    {
        ...
        // The overlap between this field and the window, as whole cells in each. Both are on the same lattice, so this is a copy rather than a resample - which is the reason for snapping the window
        // to the cell size the fields are already using.
        ...
        // The surface's own shape, read fresh rather than cached on the Field: it is geometry, already owned by mGeometry, so copying the slope into the integrator alongside wet/snow/puddle would
        // only be a second copy of data that already exists and could go stale between the two.
        auto geom_it = mGeometry.find(entry.first);
        ...
        for (S32 wy = y0; wy < y1; ++wy)
        {
            ...
            for (S32 wx = x0; wx < x1; ++wx)
            {
                ...
                {
                    // Which way water on this cell runs, and how hard it is being pushed to run: the surface's own downslope direction, and how steep it is against how much rain is actually landing
                    // here. That product is the whole directionality model - a steep roof in a downpour streaks hard, the same roof in a drizzle barely does, and a level floor never does however wet
                    // it gets.
    ...
    if (mWindowTex == 0 || mWindowRes != WINDOW_RES)
    {
        ...
        // Immutable storage is GL 4.2. Everything else here is older than that, so this is the one call that can be missing on a driver the rest of the system is perfectly happy on.
        ...
        // Drain whatever is already queued before allocating anything. GL errors are sticky: glGetError reports the OLDEST unretrieved error and clears one flag per call, so a failure raised
        // anywhere earlier in the frame is still sitting there when the check below runs. It then attributes somebody else's mistake to these two allocations, tears the field down and leaves nothing
        // shading wet for the rest of the session - over a texture that was created perfectly well. Cheap insurance, and the only way for this check to mean what it says: after this loop, any error
        // the check sees is genuinely ours.
        while (glGetError() != GL_NO_ERROR) { }

        glGenTextures(1, &mWindowTex);
        glBindTexture(GL_TEXTURE_2D, mWindowTex);

        // Linear, so the wetness under a fragment varies smoothly rather than in metre squares. It softens the height channel across a roof edge too, over the one cell the edge falls in, and that
        // reads as the lip of the shelter being soft - which is nearer the truth than a step.
        ...
        // Linear, like the main window. This used to be nearest, because a routed flow field genuinely could not be interpolated: two channels meeting head-on in a roof valley blend to a shrunken
        // vector that points somewhere between two real answers and means neither. A gradient field has no such discontinuities - neighbouring cells on one surface slope almost the same way, by
        // construction - so it interpolates cleanly, and the hard cell edges that idiom cost are gone with it.
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
...
bool SSSurfaceField::bindForShader(LLGLSLShader& shader, S32 channel)
{
    ...
    // xy the agent-space corner of texel zero, z the metres a cell spans, w the resolution - between them everything a lookup needs
...
bool SSSurfaceField::bindFlowForShader(LLGLSLShader& shader, S32 channel)
{
    ...
    // Same lattice as the main window - ssFieldOrigin, already bound by bindForShader, is all either texture needs to be sampled.
    return true;
}
...
{
    // A pass that quietly declines to run looks exactly like one that runs and changes nothing, and the two want completely different fixes. Each gate says so once, and says so again only when the
    // answer changes, so a log from a whole session is a handful of lines rather than a flood.
    ...
    if (mScratch.getWidth() != w || mScratch.getHeight() != h)
    {
        ...
        // allocate() returning true is not the same as the target actually holding a texture - addColorAttachment can decline quietly further down. Caught here, once, rather than discovered as a
        // getTexture() warning three calls later with no context left to explain it.
    ...
    // Binds the gbuffer's own attachments as the diffuse, specular and normal samplers, along with the depth. The specular one is what this pass is rewriting, which is exactly why it is being
    // rewritten into the scratch target rather than in place.
    gPipeline.bindDeferredShader(gSSSurfaceWetProgram);

    // The field's own sampler goes after everything the deferred bind claimed, which is what mActiveTextureChannels is counting. Bound by hand rather than through enableTexture, so it has to be let
    // go by hand too - the deferred unbind below only knows about the channels it claimed itself.
    const S32 field_channel = gSSSurfaceWetProgram.mActiveTextureChannels;
    bindForShader(gSSSurfaceWetProgram, field_channel);

    // Avatar capsules, uploaded whether or not there are any: the shader reads ssAvatarCount before anything else, and a stale count from a previous frame would have it testing capsules that are no
    // longer there.
    ...
    // Turbulence spreads the fall direction, and that spread is what softens the edge of an overhang. Tried tying this to the gust the wind system tracks, on the reasoning that a squall should blur
    // the shelter line more than still air does - but a value that keeps changing frame to frame makes a perfectly static edge visibly breathe, which reads as a bug even though the reasoning behind
    // it was sound. A single fixed width holds still, and that matters more here than the physical justification did.
    ...
    // Left as settings rather than constants because what reads as wet is a judgement about content, not a physical figure: a region of matte legacy prims and one of authored PBR want different
    // numbers out of the same rain. Winding the roughness pair to zero is also the quickest way to find out whether this pass is running at all.
    ...
    // Terrain, and any legacy content like it, carries no baked specular at all - checked in the shader rather than here, since it depends on the gbuffer's own value at each fragment, not anything
    // known on this side.
    ...
    // Diagnostic override: soaks every fragment the pass reaches, whatever the field says. Separates "the pass writes nothing anyone can see" from "the field is not reaching the shader", which look
    ...
    // Diagnostic: real field lookup, exposure march skipped. Isolates the field texture/coordinate path from the shelter logic.
    ...
    // Same field, same wet value, a second independent pass: flattens the shading normal toward up on the same surfaces the pass above just brightened, tapered off on anything steep enough that
    // water on it runs as a sheet rather than pooling flat. Gated on its own shader having built, separately from the wetness one above - a compile failure here costs the flattening, not the wetness
    // the surfaces already have.
    ...
    if (do_normal && mScratchNormal.getNumTextures() >= 1)
    {
        ...
        // The same tileable wave-normal texture the water plane itself uses for ripples - fetched by UUID each pass rather than cached on this object, the same way LLDrawPoolWater picks it up, so a
        // region changing its water settings changes what streams look like too without this needing to be told. The texture manager's own cache keyed on that UUID is what keeps this cheap.
        ...
        // Degrees from vertical-up, converted to cosines here rather than in the shader so a per-pixel inverse cosine is never needed - the shader already has a cosine on hand in the surface's own
        // normal dotted with up, and comparing that against another cosine is exactly the smoothstep it already wanted to do.
        ...
        // Same standing-depth figure the wetness pass just used, and its own flatten amount rather than a share of ssWetNormalFlatten - a puddle is meant to read as fully level water at full depth
        // regardless of how far the ordinary wet-wall taper is dialled.
        ...
        // atmo->sharedTime() is seconds since the Unix epoch - a fine clock for scheduling, but a poor one to hand a shader: at that magnitude a 32-bit float's precision has already coarsened to
        // better than a minute per step, which is exactly why nothing here looked like it was moving. gFrameTimeSeconds is the same continuous clock the rest of the viewer already animates against,
        // rebased to zero at launch so it stays precise for hours.
        ...
        // No wave texture bound leaves ssWaveMap sampling whatever texture unit wave_channel happened to hold last, so the strength that blends its result in has to drop to zero along with it rather
        // than trusting the setting alone. Scaled by the wind, so standing water shivers when a gust crosses it and lies glassy in a lull. Cheap in the truest sense: the shader already animates the
        // wave normal, and this only decides how hard - so a gust becomes visible ON THE GROUND, through a channel (the gust envelope) that until now only the ears got.
    ...
    // The scratch now holds what the specular attachment should become. Put it back by drawing into the gbuffer with every attachment but that one masked off, rather than by copying into the
    // texture. The copy this replaces named its destination by way of whichever texture unit happened to be active, and getting that unit right meant agreeing with the renderer's binding cache about
    // state it is entitled to skip work over. A draw names the destination through the framebuffer, which nothing caches and nothing can quietly decline to do.
    ...
    // Same diagnostic idea as the debug force above, one stage further down the pipe: paints diffuse magenta from the commit pass itself, so a mechanism failure here shows up whether or not the
    // wetness pass upstream of it wrote anything sensible to the scratch target.
    ...
    // Ground truth. Everything checked so far says the draw succeeded and landed nowhere visible, which is a contradiction - one of those two things is not actually true. Reading the pixel back,
    // while this FBO and this draw buffer are still current, settles which: it names the exact texture object the read comes from, so there is no scope left for "the write worked but not into what
    // the screen shows" to hide in.
    if (ssCommitDebugPaint > 0.f)
    {
        ...
        // Five points rather than one: if a leftover scissor rect clipped the draw to a corner of the screen, sampling only the centre could land inside it by chance and report a false all-clear. A
        // pattern across corners and centre cannot be faked by a single narrow rect.
        const S32 inset = 4;
    ...
    // Same shader, same draw, a different source and a different mask: the commit shader only ever does "copy this texture into whichever attachment glDrawBuffers points frag_data[1] at", which is
    // exactly as true for the normal buffer as it was for the specular one. Nothing about this shader needed to know there would be a second caller.
    if (do_normal && mScratchNormal.getNumTextures() >= 1)
    {
    ...
    // Handed back with every attachment live again, so the next thing to bind this target does not inherit a mask it never asked for
...
void SSSurfaceField::renderDebug()
{
    ...
    for (const auto& entry : mFields)
    {
        ...
        for (S32 i = 0; i < (S32)fld.mZ.size(); ++i)
        {
            ...
            // Blue for wet, white for snow, cyan and opaque for standing water, so which of the three a cell is carrying is legible at a glance rather than needing the depth read off a number
            F32 r, g, b, a;
            if (puddle > 0.001f)
            {
