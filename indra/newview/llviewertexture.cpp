...
void LLViewerFetchedTexture::readbackRawImage()
{
        ...
        (mRawImage.isNull() || mRawImage->getWidth() < mGLTexturep->getWidth() || mRawImage->getHeight() < mGLTexturep->getHeight() ))
    {
        ...
        // <SS:Nexii> Prefer the level-0 mip - a celestial disc rendered small keeps coarse GL
        // mips, and a current-level readback averages the alpha falloff until the disc-padding
        // analyzer's 90% contour washes out to full-bleed. Level 0 is the authored pixels. Only
        // when level 0 is not resident (the texture is still fetching or lives at a coarser
        // discard) fall back to the current displayed level so the call still delivers data.
