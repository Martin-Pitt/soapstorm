...
- Geometry: ONE canonical frame lattice in units of the rect's half extent (32 cells per half extent ≈ 32 m cells at a 2048 m footprint — stock water's own subdivision step; frame half extent 128 cells = 4× the footprint), built once, never rebuilt. Cells continue outward from the rect in equal world steps: square cells, undistorted UVs, stock-identical wave interpolation, and the waves are world-anchored by the rect itself — no snapped `ss_sea_origin`, zero swimming; cells only re-step when the rect changes (region cross, neighbour connect), a one-off. The 128/32 ratio is baked into waterV's chebyshev normalisation (0.25) — change one, change both.
- The horizon is a BLEND BAND, not a remap: from blend start (ss_sea.x, a chebyshev fraction of the frame) to the outer square edge, each vertex smoothsteps 0→100% from its lattice position toward the camera-centred rim circle along its own camera ray, so the square's edge lands exactly on the round horizon (~1024 points around) and the shared squash band (knee→cap over knee→rim) carries the drawn depth. Blend start = clamp(knee/(4·min half extent), 0.3, 0.85): MESH-anchored, never camera-distance-anchored — a camera-distance ramp gave far-side seam vertices a few percent of a 100 km pull and tore kilometres off the rect edge; the clamp floor keeps the blend 0.2 half-extents clear of the seam, the ceiling guarantees a band when a huge knee outruns the 4 km frame. Everything that varies — camera, blend start, rim, water height, sink, droop, hole rect — is a uniform.
...
- Squash: binds `ss_squash = (knee, cap, r_sea)` around its own draw only and zeroes it after — the uniform's zero default means stock planes through the same SS_ATMO-compiled shader stay byte-for-byte unsquashed. Knee/cap come from SSVolCloud so water, cloud and lightning agree about drawn depth; the sea's virtual radius is its own (the horizon may be far beyond the cloud field).
